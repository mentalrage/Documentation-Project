...
- Generated-data caveat: current `simroot_v2` metadata references `CheckTimerSkewAndSendHeartbeat_00466CA0.cpp`, but that recovered source file is not present in the tree.
...
The heartbeat packet branch is guarded by the packet sender, a session/object pointer at `dword_67A748`, and the connection-closed/reconnect dialog singleton fields in [UID:00029B][0x0067ab54-0x0067ab84.ApplicationDialogTimerAndBackgroundGlobals](by-memory/0x0067ab54-0x0067ab84.ApplicationDialogTimerAndBackgroundGlobals.md). It initializes a packet buffer, writes first byte `0x60`, mixes `GetTickCount()` and `rand()` bytes, appends obfuscated/randomized bytes derived from `dword_67A748 + 0x1a0`, then queues 11 bytes through [UID:0001HU][0x00574bb0-0x00574c13.QueueAndSendPacket](by-memory/0x00574bb0-0x00574c13.QueueAndSendPacket.md).
...
- Current filesystem check found no `source-3/simroot_v2/recovered/CheckTimerSkewAndSendHeartbeat_00466CA0.cpp`; keep prior generated references to that path as stale lead data until Wave3/simroot output is reconciled.
...
- 2026-05-30: Grading changed from unevaluated `0/0` to `82/88`.
  - Before: page summarized the helper but did not carry completion/confidence scoring and referenced generated recovered output too broadly.
  - After: page records the current IDA-backed function boundary, caller, packet construction, gating globals, and stale generated-file caveat.
