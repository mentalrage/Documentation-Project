...
Singular `by-function` pages are eligible for validator autogen metadata when they contain final C++ for a free function, helper, or method stub that should be assembled into rebuilt source. Prefer keeping the full body in `../by-memory`; use `by-function` autogen only when the name-centric page is the active reconstruction owner. Use `CANONICAL_OWNER` for the direct semantic owner, such as the owning class UID for methods or the owning `by-file` UID for file-level helpers. Use `EMITTER_UIDS` for generated-output routing and make sure the emitter chain reaches a valid by-file source root before adding emitted C++. Add emitted C++ only when the active `90/90+` reconstruction-code gate in `../by-structure.md` is satisfied; `95+` remains a rare final-audit score and is not required merely to begin final-quality code entry.
