...
- Current Wave3 alias: `g_activeGameServerConfig`.
...
`g_activeMapPane` is the process-wide pointer to the live map pane. Wave3 currently exposes it through the misleading `g_activeGameServerConfig` name because the `0x00504110` map-pane initializer is attributed to `GameServerConfig`.
...
- Weather layer drawing in [UID:0000MY][RainingLayerPane](by-file/RainingLayerPane.md) and [UID:0000NR][SnowingLayerPane](by-file/SnowingLayerPane.md) also checks byte `dword_67A764 + 1008`; current generated output aliases this same storage as `g_pWeatherState`.
