...
- Confidence: strong for browser timeout alert text role, browser-module ownership, and duplicate-alias disposition; medium-high for final source literal name.
...
- Current memory evidence: [UID:0001OD][0x00613a20-0x00613ab0.BrowserAlertStrings](by-memory/0x00613a20-0x00613ab0.BrowserAlertStrings.md), direct UTF-16 string `Navigation Timeout`.
...
Generated `BrowserPane::OnCommand` uses `g_pBrowserTimeoutText` for command id `1111716975` (`BCto` in older notes). IDA decompilation resolves that generated symbol to the direct string literal at `0x00613a54`, text `Navigation Timeout`. The same command id appears in `BrowserControlPane::HandleBrowserDispatchEvent` as the document-complete callback token sent to the active browser pane.
...
This is browser UI text/static state for timeout/document-complete alert handling. It should stay with `BrowserPane` and the browser command constants.
...
- 2026-06-03 autogen attachment pass:
  ...
  - Evidence: [UID:0000HV][Browser](by-file/Browser.md) is `84/88`; [UID:0001OD][0x00613a20-0x00613ab0.BrowserAlertStrings](by-memory/0x00613a20-0x00613ab0.BrowserAlertStrings.md) is `82/88`, reconstructable, attached to Browser, and records the IDA-confirmed `0x00613a54-0x00613a7a` `Navigation Timeout` literal used by `BrowserPane::OnCommand`. C++ remains blank because this page is a generated alias note, not a separate proven pointer-storage definition, and final literal naming is below the `95+` code-emission gate.
