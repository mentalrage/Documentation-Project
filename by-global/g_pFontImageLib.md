...
- Common generated alias caveat: some text consumers have been emitted as `g_pDirectX` or raw `char*` users even though IDA shows they read `dword_67AB24`.
...
`g_pFontImageLib` is source-owned by `FontImageLib`, not by `GrafPort`, `DirectX`, `TextButtonControlPane`, or individual UI controls. Generated `g_pDirectX` references in font measurement paths should be treated as the known adjacent-global alias issue unless IDA proves a specific site reads `0x0067ab20` instead of `0x0067ab24`.
