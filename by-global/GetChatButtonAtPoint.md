...
IDA disassembly shows the caller pushes the event coordinate fields directly, so keep this helper modeled with a coordinate-style signature rather than as a `this` plus event-object wrapper.

## Source Placement

Treat this as a private helper in `social/Chatting.cpp`, or as a private/static helper beside `ChattingHandlePane` if the chat-handle code is split out. Do not infer an original standalone `GetChatButtonAtPoint.cpp` source file from the helper name alone.
