...
Singular global pages are eligible for validator autogen metadata when they contain source-level global, static, singleton, table, vtable, or callback declaration/definition code that should appear in rebuilt C++. Attach file-local globals to the owning `by-file` UID. Attach class-owned static storage or vtables to the owning class UID only when that ownership is well supported.
