...
- Current generated name: `g_activeUserStatusPane`
...
Several inventory/user-pane paths also read fields through this same pointer. For example, [UID:0000LO][MyItemListPane](by-file/MyItemListPane.md) reads the inventory slot count from `dword_69AE0C + 0x284` before enumerating item slots through the local-player slot-address helper. Treat generated names that assign this storage to `g_pEffectObjImageLib` or raw `DAT_0069ae0c` as alias pollution.

[UID:00001J][ChangeItemSlotInputPane](by-class/ChangeItemSlotInputPane.md) also reads byte offset `+0x284` from this singleton while validating typed item-slot letters. Current generated wording may call the receiver `g_pLocalPlayer`, but IDA address evidence still points to this `dword_69AE0C` storage.
...
- IDA reports many broader UI xrefs, so the generated name should remain provisional until final global naming review.
