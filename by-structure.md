...
`AUTOGEN_PARENT_UID` must point to the correct direct semantic parent, not merely to the nearest `by-file` page or the final generated `.cpp` root. Use the narrowest true owner that the evidence supports and that clears the assignment gate. If a vtable, nested type, static member, method group, or helper belongs directly to a class, attach it to the `by-class` page; that class then attaches to its owning `by-file` page. If an item is a file-level free function, source-module helper, global/static declaration, table, or file-owned data object with no narrower class/type owner, attach it directly to the `by-file` page. If the correct parent is a global/type/class/resource page, use that parent when the model supports it and document any tooling limitation if it does not. Do not bypass the correct class/type/global parent just to make the item reach a file root sooner.

Parenting is a documentation ownership claim, not an output-routing shortcut. A child may ultimately emit through a file because its class or other parent emits through that file, but the child should still attach to the direct owner. For example, a vtable whose RTTI names `IntAlphaSurface` should attach to the `IntAlphaSurface` class page if that is the direct owner; the class page then attaches to the `IntAlphaSurface` file page. Use a `by-file` fallback only when the actual direct owner is the file/module or when the validator/tooling cannot represent the correct semantic parent, and explicitly document that workaround and the better intended parent in the affected page.
...
Use `RECONSTRUCTABLE:FALSE` for a reviewed container that is only an audit/index/section map, compiler/runtime grouping, padding/tail-fill range, broad address-neighborhood map, or mixed-owner convenience page whose exact children already carry the real source ownership. This does not make the children non-reconstructable. It only means the parent container should not emit or own source as a separate source-level item. For example, a broad `by-memory` range that merely groups unrelated exact child functions from `VectorHelpers`, `StringUtil`, and a UI class should normally be `FALSE` after the exact child pages are split and assigned, because the parent is an audit/container page rather than an original source owner.
