*** UID:0000BA | DO NOT MODIFY OR REMOVE!!! ***
...
// Concrete ProtectedArray<MantleInfo> template instantiation.
...
# ProtectedArray_struct_MantleInfo_
...
- Source-level name: `ProtectedArray<MantleInfo>`
- Source-level owner/emitter: [UID:0000MU][ProtectedArray](by-file/ProtectedArray.md) reusable template support.
- Concrete embedder: [UID:0000JY][HumanImageLib](by-file/HumanImageLib.md), with `ProtectedArray<MantleInfo> mantleArray` at `HumanImageLib +0x1e0` in [UID:0001UR][HumanImageLibLayout](by-type/by-struct/HumanImageLibLayout.md).
- Current generated output: 2026-06-16 `rg --files` found no active `source-3/simroot_v2/class_ProtectedArray_struct_MantleInfo_.cpp`; the only non-doc matches were Wave2 JSON metadata, which this pass did not use as authoritative source evidence.
...
- Final C++ remains blank because the exact `ProtectedArray<T>` declaration/header basename, final `MantleInfo` source declaration, and concrete member declaration wording are not source-quality yet.

## Methods

| Address | Method | Notes |
| --- | --- | --- |
| `0x004e5b30` | ordinary destructor | 16-byte interior body (Verified with int_convert.py); resets the `ProtectedArray<MantleInfo>` vtable and frees backing storage. |
| `0x004e61e0` | scalar deleting destructor | Standalone `0x2f` / 47-byte deleting body (Verified with int_convert.py); ordinary cleanup plus conditional `operator delete`. |

## Ownership Notes

This is old-human mantle metadata support, not an independent original class source file. The reusable destructor/vtable artifacts belong to the [UID:0000MU][ProtectedArray](by-file/ProtectedArray.md) template route. [UID:0000JY][HumanImageLib](by-file/HumanImageLib.md) remains the concrete owner for table loading and the embedded `mantleArray` member.

## Evidence Notes

[UID:0001WR][ProtectedArrayTemplates](by-type/by-template/ProtectedArrayTemplates.md) records `ProtectedArray<MantleInfo>` at ordinary destructor `0x004e5b30` and deleting destructor `0x004e61e0`. [UID:00017V][0x004e5a70-0x004e5b6f.ProtectedArrayImageInfoOrdinaryDestructors](by-memory/0x004e5a70-0x004e5b6f.ProtectedArrayImageInfoOrdinaryDestructors.md) identifies `0x004e5b30-0x004e5b3f` as the exact ordinary destructor body, and [UID:000187][0x004e5fa0-0x004e629f.ProtectedArrayImageInfoDeletingDestructors](by-memory/0x004e5fa0-0x004e629f.ProtectedArrayImageInfoDeletingDestructors.md) identifies `0x004e61e0-0x004e620f` as the exact deleting destructor body. [UID:0001XQ][HumanImageLibVtable](by-type/by-vtable/HumanImageLibVtable.md) records the `ProtectedArray<MantleInfo>` vtable at `0x0061b6bc` with slot target `0x004e61e0`.

2026-06-16 A001 live IDA MCP refresh used active database `b001_mappane_0001AW_20260616` for read-only `NexusTK.exe`. `lookup_funcs` resolves `0x004e5b30` inside broad `sub_4D2720` (`0x23aa` / 9130 bytes, Verified with int_convert.py), confirms standalone `sub_4E61E0` at `0x004e61e0` (`0x2f` / 47 bytes, Verified with int_convert.py), and reconfirms the HumanImageLib constructor/destructor anchors `0x004d2720` and `0x004d4ad0`. `xrefs_to` reports the ordinary-destructor cleanup/unwind ref at `0x005ffbf6`, the deleting-destructor vtable data ref from `0x0061b6bc`, and four vtable-operand refs from constructor embed `0x004d29b7`, destructor cleanup `0x004d4e0c`, ordinary reset `0x004e5b33`, and deleting reset `0x004e61e9`.

The same pass decompiled `0x004e61e0` and confirmed the scalar deleting shape: read/free `Block[2]`, write `ProtectedArray<MantleInfo>::vftable`, test delete flag bit `1`, optionally call the delete helper, and return `this`. A read-only section-mapped PE scan found no absolute/RVA pointers to the ordinary destructor, one `.text` rel32 jump at `0x005ffbf6`, one absolute vtable-slot pointer to the deleting destructor at `0x0061b6bc`, no rel32 calls/jumps to `0x004e61e0`, and four absolute vtable-operand pointer locations `0x004d29bd`, `0x004d4e12`, `0x004e5b35`, and `0x004e61eb`. Byte reads also show the deleting body uses delete-size immediate `0x20` / 32 bytes (Verified with int_convert.py).

Blockers checked: IDA `search_structs` returned no local records for `ProtectedArray`, `MantleInfo`, or `HumanImageLib`, and generated-file search found no active `simroot_v2` C++ file for this instantiation. Current IDA/docs/PE evidence proves the binary artifact and owner route, but not the original template declaration spelling, header basename, final `MantleInfo` declaration, or final HumanImageLib member declaration text.
...
- 2026-06-29 B002 direct empty-emitter implementation:
  ...
  - Added the formal no-code `ProtectedArray<MantleInfo>` instantiation comment using corrected embedder [UID:0000JY][HumanImageLib](by-file/HumanImageLib.md).
...
- 2026-06-05: Changed `RECONSTRUCTABLE` from blank to `TRUE` and left `AUTOGEN_PARENT_UID` blank.
  - Before: The class was still unclassified in autogen coverage even though the page identified `ProtectedArray<MantleInfo>` as NexusTK-owned template support embedded by `HumanImageLib`.
  ...
  - Evidence: Live IDA MCP lookup confirms the ordinary cleanup entry at `0x004e5b30` and deleting destructor at `0x004e61e0`; linked template and memory docs identify the shared `ProtectedArray<T>` rebuild requirement.
...
- 2026-05-30: Changed completion/confidence from `0/0` to `70/86`.
  ...
  - After: Scored as adequate completion and strong confidence for a generated `ProtectedArray<MantleInfo>` instantiation.
