*** UID:0000AZ | DO NOT MODIFY OR REMOVE!!! ***
...
// Concrete ProtectedArray<BodyInfo> template instantiation.
...
# ProtectedArray_struct_BodyInfo_
...
- Source-level name: `ProtectedArray<BodyInfo>`
- Source-level owner/emitter: [UID:0000MU][ProtectedArray](by-file/ProtectedArray.md) reusable template support.
- Concrete feature embedder: [UID:0000JY][HumanImageLib](by-file/HumanImageLib.md), with the embedded field recorded as `bodyArray` at `HumanImageLib +0x03c` by [UID:0001UR][HumanImageLibLayout](by-type/by-struct/HumanImageLibLayout.md).
- Current generated file: `source-3/simroot_v2/class_ProtectedArray_struct_BodyInfo_.cpp`
...
- Output route: `CANONICAL_OWNER` and `EMITTER_UIDS` both route to [UID:0000MU][ProtectedArray](by-file/ProtectedArray.md). `RECONSTRUCTION_CPP` remains blank even though the current combined-score/emitter gate is satisfied, because the final template declaration, header basename, and source-facing `BodyInfo` field/member declarations are not source-quality yet.

## Methods

| Address | Method | Notes |
| --- | --- | --- |
| `0x004e5a90` | ordinary destructor | Resets `ProtectedArray<BodyInfo>` vtable and frees backing storage. |
| `0x004e6000` | scalar deleting destructor | Ordinary cleanup plus conditional `operator delete`. |

## Ownership Notes

This is old human body-part metadata support, not an independent original class source file. The reusable destructor/deleting-destructor source shape belongs to `ProtectedArray<T>`, while `HumanImageLib` owns the concrete `BodyInfo` records, embedded field, and old-human body table integration.

## Evidence Notes

[UID:0001WR][ProtectedArrayTemplates](by-type/by-template/ProtectedArrayTemplates.md) records `ProtectedArray<BodyInfo>` at ordinary destructor `0x004e5a90` and deleting destructor `0x004e6000`. [UID:00017V][0x004e5a70-0x004e5b6f.ProtectedArrayImageInfoOrdinaryDestructors](by-memory/0x004e5a70-0x004e5b6f.ProtectedArrayImageInfoOrdinaryDestructors.md) identifies `0x004e5a90-0x004e5a9f` as the exact ordinary destructor body, and [UID:000187][0x004e5fa0-0x004e629f.ProtectedArrayImageInfoDeletingDestructors](by-memory/0x004e5fa0-0x004e629f.ProtectedArrayImageInfoDeletingDestructors.md) identifies `0x004e6000-0x004e602f` as the exact deleting destructor body. [UID:0001XQ][HumanImageLibVtable](by-type/by-vtable/HumanImageLibVtable.md) records the `ProtectedArray<BodyInfo>` vtable at `0x0061b67c` with slot target `0x004e6000`.

2026-06-16 A001 live IDA/PE refresh:

- `lookup_funcs` in IDA database `b001_mappane_0001AW_20260616` still resolves `0x004e5a90` inside broad `sub_4D2720` (`0x23aa` bytes), while `0x004e6000` is an exact `0x2f` / 47-byte function (Verified with int_convert.py).
- `xrefs_to 0x004e5a90` reports one rel32 cleanup/unwind-style code reference at `0x005ffb1c`; the read-only PE scan found the same single rel32 reference and no absolute-VA or RVA pointer hits to the ordinary-destructor start.
- `xrefs_to 0x004e6000` reports the concrete vtable data reference at `0x0061b67c`; the PE scan found one absolute-VA hit at the same vtable slot and no rel32 calls/jumps to the deleting destructor.
- `xrefs_to 0x0061b67c` reports four references: HumanImageLib constructor embed/init at `0x004d27c9`, HumanImageLib destructor cleanup at `0x004d4f14`, the ordinary destructor vtable reset at `0x004e5a93`, and the scalar deleting destructor vtable reset at `0x004e6009`.
- Decompilation of `0x004e6000` resets the `ProtectedArray<BodyInfo>` vtable, frees `Block[2]` / object `+0x08`, tests delete flag bit `1`, conditionally calls delete support, and returns `this`.
- `get_bytes` reconfirmed the first three ordinary destructor bodies as consecutive `0x10` / 16-byte bodies (Verified with int_convert.py); the BodyInfo body writes vtable `0x0061b67c`, calls free, and returns.
...
| Confidence `91` | IDA and PE evidence agree on the function boundaries, vtable slot, cleanup reachability, HumanImageLib embedding, and corrected no-code disposition. Confidence stays below final-audit range because no IDA type record or source-facing declaration evidence proves the original `ProtectedArray<T>` header/member names or `BodyInfo` field declarations. |
...
- 2026-06-29 B002 direct empty-emitter implementation:
  ...
  - Added the formal no-code `ProtectedArray<BodyInfo>` instantiation comment using corrected embedder [UID:0000JY][HumanImageLib](by-file/HumanImageLib.md).
...
- 2026-06-05: Marked `RECONSTRUCTABLE:TRUE` and left routing blank under the older parent-field schema.
  - Before: reconstruction autogen classification was blank for a documented custom `ProtectedArray<BodyInfo>` instantiation.
  - After: classified as reconstructable source-facing template/declaration evidence, not as a standalone original `.cpp` file.
  - Evidence: [UID:0001WR][ProtectedArrayTemplates](by-type/by-template/ProtectedArrayTemplates.md) is already `RECONSTRUCTABLE:TRUE`, and this page ties `ProtectedArray<BodyInfo>` to [UID:0000JY][HumanImageLib](by-file/HumanImageLib.md). Live IDA MCP lookup confirms deleting destructor `0x004e6000` and resolves ordinary destructor address `0x004e5a90` inside the documented old-human image-library destructor island. Parent attachment was deferred at that score and superseded by the 2026-06-06 attachment above after the page met the 80/80 gate.
...
- 2026-05-30: Changed completion/confidence from `0/0` to `70/86`.
  ...
  - After: Scored as adequate completion and strong confidence for a generated `ProtectedArray<BodyInfo>` instantiation.
