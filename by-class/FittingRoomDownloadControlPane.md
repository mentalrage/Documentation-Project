...
| Score | Rationale |
| --- | --- |
| Completion `70` | The class has confirmed lifecycle bodies, a deleting destructor, local progress state linkage, and parent source ownership. Completion remains capped because the exact `OnPaint` by-memory split is still inside the broad UI core page and several generated attachments are known owner-pollution candidates. |
| Confidence `82` | Vtable writes, vtable restoration, shared teardown, and fitting-room parent evidence are direct and consistent. Confidence is not higher because the raw constructor has no IDA function record/xrefs and final member names are not proven. |
