...
// Concrete ProtectedArray<HelmetInfo> template instantiation.
...
- Concrete embedder: [UID:0000JY][HumanImageLib](by-file/HumanImageLib.md), with `ProtectedArray<HelmetInfo> helmetArray` at `HumanImageLib +0x1c0` in [UID:0001UR][HumanImageLibLayout](by-type/by-struct/HumanImageLibLayout.md).
- Current generated output: 2026-06-16 `rg --files` found no active `source-3/simroot_v2/class_ProtectedArray_struct_HelmetInfo_.cpp`; the only non-doc matches were Wave2 JSON metadata, which this pass did not use as authoritative source evidence.
...
- Final C++ remains blank because the exact `ProtectedArray<T>` declaration/header basename, final `HelmetInfo` source declaration, and concrete member declaration wording are not source-quality yet.

## Methods

| Address | Method | Notes |
| --- | --- | --- |
| `0x004e5b00` | ordinary destructor | 16-byte interior body; resets the `ProtectedArray<HelmetInfo>` vtable and frees backing storage. |
| `0x004e6150` | scalar deleting destructor | Standalone `0x2f` / 47-byte deleting body; ordinary cleanup plus conditional `operator delete`. |

## Ownership Notes

This is old-human helmet metadata support, not an independent original class source file. The reusable destructor/vtable artifacts belong to the [UID:0000MU][ProtectedArray](by-file/ProtectedArray.md) template route. [UID:0000JY][HumanImageLib](by-file/HumanImageLib.md) remains the concrete owner for table loading and the embedded `helmetArray` member.
...
2026-06-16 A001 live IDA MCP refresh used active database `b001_mappane_0001AW_20260616` for read-only `NexusTK.exe`. `lookup_funcs` resolves `0x004e5b00` inside broad `sub_4D2720` (`0x23aa` bytes), confirms standalone `sub_4E6150` at `0x004e6150` (`0x2f` bytes), and reconfirms the HumanImageLib constructor/destructor anchors `0x004d2720` and `0x004d4ad0`. `xrefs_to` reports the ordinary-destructor cleanup/unwind ref at `0x005ffbe5`, the deleting-destructor vtable data ref from `0x0061b6b4`, and four vtable-operand refs from constructor embed `0x004d298f`, destructor cleanup `0x004d4e21`, ordinary reset `0x004e5b03`, and deleting reset `0x004e6159`.

The same pass decompiled `0x004e6150` and confirmed the scalar deleting shape: read/free `Block[2]`, write `ProtectedArray<HelmetInfo>::vftable`, test delete flag bit `1`, optionally call the delete helper, and return `this`. A read-only section-mapped PE scan found no absolute/RVA pointers to the ordinary destructor, one `.text` rel32 jump at `0x005ffbe5`, one absolute vtable-slot pointer to the deleting destructor at `0x0061b6b4`, no rel32 calls/jumps to `0x004e6150`, and the four absolute vtable-operand pointer locations `0x004d2995`, `0x004d4e27`, `0x004e5b05`, and `0x004e615b`. Byte reads also show the deleting body uses delete-size immediate `0x20`.

Blockers checked: IDA `search_structs` returned no local records for `ProtectedArray`, `HelmetInfo`, or `HumanImageLib`, and generated-file search found no active `simroot_v2` C++ file for this instantiation. Current IDA/docs/PE evidence proves the binary artifact and owner route, but not the original template declaration spelling, header basename, final `HelmetInfo` declaration, or final HumanImageLib member declaration text.
...
- 2026-06-29 B002 direct empty-emitter implementation:
  ...
  - Added the formal no-code `ProtectedArray<HelmetInfo>` instantiation comment using corrected embedder [UID:0000JY][HumanImageLib](by-file/HumanImageLib.md).
