...
// Concrete ProtectedArray<EffectInfo> template instantiation.
...
- Concrete embedder: [UID:0000IY][EffectObjImageLib](by-file/EffectObjImageLib.md), with `ProtectedArray<EffectInfo> effectInfoArray` at `EffectObjImageLib +0x04` and scratch [UID:0001U8][EffectInfo](by-type/by-struct/EffectInfo.md) at `+0x14` in [UID:0001U9][EffectObjImageLibLayout](by-type/by-struct/EffectObjImageLibLayout.md).
- Current generated output: 2026-06-16 `rg --files` found no active `source-3/simroot_v2/class_ProtectedArray_struct_EffectInfo_.cpp`; the only non-doc matches were Wave2 JSON metadata, which this pass did not use as authoritative source evidence.
...
- Final C++ remains blank because the exact `ProtectedArray<T>` declaration/header basename, final `EffectInfo` source declaration, and concrete member declaration wording are not source-quality yet.

## Methods

| Address | Method | Notes |
| --- | --- | --- |
| `0x004e5ab0` | ordinary destructor | 16-byte interior body; resets the `ProtectedArray<EffectInfo>` vtable and frees backing storage. |
| `0x004e6060` | scalar deleting destructor | Standalone `0x2f` / 47-byte deleting body; ordinary cleanup plus conditional `operator delete`. |

## Ownership Notes

`EffectObjImageLib` owns the effect descriptor load/lookup behavior and the `EFFECT.TBL` record declaration. This page records only the shared `ProtectedArray<T>` instantiation artifacts, so [UID:0000MU][ProtectedArray](by-file/ProtectedArray.md) remains the canonical owner/emitter.
...
2026-06-16 A001 live IDA MCP refresh used active database `b001_mappane_0001AW_20260616` for read-only `NexusTK.exe`. `lookup_funcs` resolves `0x004e5ab0` inside broad `sub_4DDF60` (`0xee` bytes), confirms standalone `sub_4E6060` at `0x004e6060` (`0x2f` bytes), and reconfirms the `EffectObjImageLib` constructor/destructor anchors `0x004ddf60` and `0x004de050`. `xrefs_to` reports the ordinary-destructor cleanup/unwind ref at `0x00600095`, the deleting-destructor vtable data ref from `0x0061b714`, and five vtable-operand refs from constructor embed `0x004ddfb6`, ordinary destructor cleanup `0x004de158`, ordinary reset `0x004e5ab3`, deleting reset `0x004e6069`, and `EffectObjImageLib` scalar deleting cleanup `0x004e63ff`.

The same pass decompiled `0x004e6060` and confirmed the scalar deleting shape: read/free `Block[2]`, write `ProtectedArray<EffectInfo>::vftable`, test delete flag bit `1`, optionally call the delete helper, and return `this`. Decompilation of `0x004ddf60` and `0x004de050` reconfirmed constructor/destructor writes to `this[1]` (`+0x04`) and descriptor cleanup through `this[3]` (`+0x0c`) with `EffectInfo` records stepped by `52` (`0x34`) bytes. A read-only section-mapped PE scan found no absolute/RVA pointers to the ordinary destructor, one `.text` rel32 jump at `0x00600095`, one absolute vtable-slot pointer to the deleting destructor at `0x0061b714`, no rel32 calls/jumps to `0x004e6060`, and the five absolute vtable-operand pointer locations `0x004ddfb8`, `0x004de15b`, `0x004e5ab5`, `0x004e606b`, and `0x004e6402`. Byte reads also show the deleting body uses delete-size immediate `0x44`, matching a `0x10` ProtectedArray header plus `0x34` byte fallback element.

Blockers checked: IDA `search_structs` returned no local records for `ProtectedArray`, `EffectInfo`, or `EffectObjImageLib`, and generated-file search found no active `simroot_v2` C++ file for this instantiation. Current IDA/docs/PE evidence proves the binary artifact and owner route, but not the original template declaration spelling, header basename, final `EffectInfo` field names, or final EffectObjImageLib member declaration text.
...
- 2026-06-29 B002 direct empty-emitter implementation:
  ...
  - Added the formal no-code `ProtectedArray<EffectInfo>` instantiation comment using corrected embedder [UID:0000IY][EffectObjImageLib](by-file/EffectObjImageLib.md).
