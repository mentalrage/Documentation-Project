...
// Concrete ProtectedArray<FaceInfo> template instantiation.
...
- Concrete feature embedder: [UID:0000JY][HumanImageLib](by-file/HumanImageLib.md), with the embedded field recorded as `faceArray` at `HumanImageLib +0x140` by [UID:0001UR][HumanImageLibLayout](by-type/by-struct/HumanImageLibLayout.md).
- Current generated metadata: `source-3/core/data/cache/wave2/classes/ProtectedArray_struct_FaceInfo_.json`; no matching `source-3/simroot_v2/class_ProtectedArray_struct_FaceInfo_.cpp` file exists in this checkout, so generated metadata is evidence only and not source-placement proof.
...
This is old human face metadata support, not an independent original class source file. The reusable destructor/deleting-destructor source shape belongs to `ProtectedArray<T>`, while `HumanImageLib` owns the concrete `FaceInfo` records, embedded field, and old-human face table integration.
...
- `lookup_funcs` in IDA database `b001_mappane_0001AW_20260616` still resolves `0x004e5ad0` inside broad `sub_4D2720` (`0x23aa` / 9130 bytes, Verified with int_convert), while `0x004e60c0` is an exact `0x2f` / 47-byte function.
- `xrefs_to 0x004e5ad0` reports one rel32 cleanup/unwind-style code reference at `0x005ffba1`; the read-only PE scan found the same single rel32 reference and no absolute-VA or RVA pointer hits to the ordinary-destructor start.
- `xrefs_to 0x004e60c0` reports the concrete vtable data reference at `0x0061b694`; the PE scan found one absolute-VA hit at the same vtable slot and no rel32 calls/jumps to the deleting destructor.
- `xrefs_to 0x0061b694` reports four references: HumanImageLib constructor embed/init at `0x004d28f1`, HumanImageLib destructor cleanup at `0x004d4e75`, the ordinary destructor vtable reset at `0x004e5ad3`, and the scalar deleting destructor vtable reset at `0x004e60c9`.
- Decompilation of `0x004e60c0` resets the `ProtectedArray<FaceInfo>` vtable, frees `Block[2]` / object `+0x08`, tests delete flag bit `1`, conditionally calls delete support, and returns `this`.
- `get_bytes` reconfirmed the Face/Hair/Head ordinary destructor bodies as consecutive `0x10` / 16-byte bodies; the FaceInfo body writes vtable `0x0061b694`, calls free, and returns.
- `search_structs ProtectedArray`, `FaceInfo`, `HairInfo`, and `HeadInfo` returned no local IDA struct/type records. The wave2 generated metadata has `has_struct:false` and `has_vtables:false`, and `rg --files` found no generated `simroot_v2` C++ file. This keeps final C++ blank: the binary proves the emitted template body and owner route, but not the original header basename, member spelling, or full source declaration surface.
...
| Confidence `91` | IDA and PE evidence agree on the function boundaries, vtable slot, cleanup reachability, HumanImageLib embedding, and corrected no-code disposition. Confidence stays below final-audit range because no IDA type record or source-facing declaration evidence proves the original `ProtectedArray<T>` header/member names or `FaceInfo` field declarations. |
...
- 2026-06-29 B002 direct empty-emitter implementation:
  ...
  - Added the formal no-code `ProtectedArray<FaceInfo>` instantiation comment using corrected embedder [UID:0000JY][HumanImageLib](by-file/HumanImageLib.md).
