...
- Rebuild handling: source-authored clan rename dialog. Attached to [UID:0000I8][Clan](by-file/Clan.md); C++ remains blank because the raw constructor is not an IDA function and the exact class declaration is still below the reconstruction-code gate.
...
- 2026-06-05 live IDA MCP reports no function object at `0x004894d0`; direct disassembly still shows constructor-shaped bytes ending in `retn 4` at `0x00489502`.
...
- 2026-06-05 live IDA MCP reports `sub_489510` as a real function at `0x00489510-0x004895fe`, with the submit callback referenced from vtable slot `0x00615d14`.
- The submitter builds a packet with byte writes for opcode `0x4b`, subtype `0x0a`, and action `1`, converts the submitted wide string through `WideCharToMultiByte`, writes a one-byte name length, copies the converted name through `sub_516220`, then sends through `dword_67A7EC` and `sub_574BB0`.
...
- Live vtable data confirms RTTI at `0x00615cb4`, primary destructor slot `0x00615cb8 -> sub_48C350`, submit slot `0x00615d14 -> sub_489510`, secondary adjustor slot `0x00615d1c -> sub_48C27B`, and tertiary adjustor slot `0x00615d4c -> sub_48C286`.
- Xrefs to the three vtable bases come from Clan dialog code at `0x00485337`/`0x0048533d`/`0x00485347`, from `sub_488370` at `0x0048842e`/`0x00488434`/`0x0048843e`, and from the raw constructor stores at `0x004894e2`/`0x004894ea`/`0x004894f4`.
