...
- Confidence: strong for `UserLookPane`, `MoreInfoPane`, and `ProfilePane` grouping; medium for the final owner of shared `LegendPane`.
...
The emitted Wave3 source splits `MoreInfoPane`, `ProfilePane`, and `LegendPane` into separate one-class files because the current recovered source model is class-first. That should not be treated as evidence that the original late-1999/mid-2000s source used four standalone `.cpp` files. For source reconstruction, `MoreInfoPane` and `ProfilePane` are stronger as local classes in `ui/panels/UserLookPane.cpp`; `LegendPane` remains a shared look-pane ownership question.
