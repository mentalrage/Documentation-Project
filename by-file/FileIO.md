...
- Evidence basis: `simroot_v2` generated source and metadata plus IDA MCP lookup/decompilation/xref checks on 2026-05-24 through 2026-05-26.
...
`StdioFile` and `PathExistsViaStat` consume the process-wide [UID:0000TQ][WideApiDispatchTable](by-global/WideApiDispatchTable.md), but they do not own it. The `_wfopen` pointer at `0x0069be14` and `_wstat64i32` pointer at `0x0069be18` are initialized by [UID:0000ML][PlatformApi](by-file/PlatformApi.md) at [UID:0000WD][0x0041a280-0x0041a4a8.WideApiDispatchInit](by-memory/0x0041a280-0x0041a4a8.WideApiDispatchInit.md).
...
| `PathExistsViaStat` | [UID:0001IZ][0x00582460-0x00582495.PathExistsViaStat](by-memory/0x00582460-0x00582495.PathExistsViaStat.md) | `util/PathUtil.cpp` or `util/StdioFile.cpp` | Shared `_wstat`-based wide-path existence check. |
...
- Generated `class_StdioFile.cpp.disabled` contains real vtable methods (`GetSize`, `GetPosition`, `Seek`, and `Read`) that active output omitted. Treat those methods as real until generated ownership is refreshed.
