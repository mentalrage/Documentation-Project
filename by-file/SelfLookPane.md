...
- Current recovered sources: `class_SelfLookPane.cpp` and `class_SelfLookPane2.cpp`
...
IDA confirms `SelfLookPane`, `SelfLookPane2`, `SpelledPane`, and `LegendPane` vtable records even though current generated metadata reports `vtable_count: 0` for each checked class. Treat generated vtable absence as a data issue, not as source-layout evidence.
...
- Do not trust the current `SelfLookPane2::ToggleBodyOption`/`ToggleFaceOption`/`ToggleHairOption` projected starts as IDA-confirmed function boundaries until the boundary mismatch is resolved.

## Evidence Notes

- Wave3 reports `SelfLookPane` grade `98.3`, `SelfLookPane2` grade `97.8`, and zero missing refs for both classes.
...
- IDA has real functions/raw starts that Wave3 currently does not model, including `0x005657e0`, `0x005693d0`, `0x00570030`, `0x00572120`, `0x005729e0`, `0x00572f30`, and `0x00572fd0`.
...
The current generated source is behaviorally useful but has boundary issues around helper/destructor functions and several `SelfLookPane2` toggle helpers. Use the IDA-confirmed function starts in the memory docs as the migration anchor, not only the active generated `.cpp` files.
