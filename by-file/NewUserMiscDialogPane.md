...
| --- | ---: | --- |
| Completion | 82 | The page now records the projected folder, ownership decision, live IDA method/range map, exact attached child page, raw helper islands, singleton evidence, dependency exclusions, and reconstruction gates. |
| Confidence | 86 | Class ownership and login/create-user placement are supported by live constructor/caller/vtable evidence, singleton storage, exact child memory coverage, and local padding/raw-island checks. Confidence is capped by the raw destructor boundary and unresolved final standalone-vs-umbrella source split. |
