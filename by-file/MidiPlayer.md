...
- Confidence: strong for behavior and source-family membership; medium for exact original split from `SoundManager`.
- Proposed module: `audio/MidiPlayer.cpp`
- Proposed header: `audio/MidiPlayer.h`
- Current generated source: `source-3/simroot_v2/class_MidiPlayer.cpp`
- Related recovered globals: `source-3/simroot_v2/recovered/ReleaseMidiBuffers_00525B10.cpp` and `source-3/simroot_v2/recovered/MidiPlayer_ResetStream_00526460.cpp`
...
- Evidence basis: `simroot_v2`, read-only Wave3 class inspection while status was `OPERATIONAL`, and IDA MCP import checks showing the WinMM MIDI API import set.
...
| --- | --- | --- | --- |
| `MidiStreamOpenAndPrimeBuffers` | omitted/helper-only | `0x00525920-0x00525b0d` | Opens WinMM stream, allocates two buffers, primes `midiStreamOut`. |
| `MidiPlayer` | `class_MidiPlayer.cpp` | `0x00525dc0-0x0052664a` sparse method island | Singleton MIDI playback controller. |
| `ReleaseMidiBuffers_525B10` | `recovered/ReleaseMidiBuffers_00525B10.cpp` | `0x00525b10-0x00525b98` | Unprepares two MIDI headers and frees backing `GlobalAlloc` buffers. |
| MIDI callback | omitted/helper-only | `0x00525be0-0x00525dbb` | WinMM callback passed to `midiStreamOpen`; handles buffer returns, refill, stop/reset events, and volume controller messages. |
| track selection/start helpers | omitted/helper-only | `0x00526100-0x00526332` | Selects DAT/config MIDI track paths, resets active stream, starts/restarts playback. |
| `MidiPlayer_ResetStream` | `recovered/MidiPlayer_ResetStream_00526460.cpp` | `0x00526460-0x00526569` | Shared stop/reset/wait/cleanup/reopen helper used by destructor, disable, stop, and callback paths. |
| MIDI file reader / SMF parser helpers | omitted/helper-only | `0x00526650-0x005270d5` | Reads file/adapter-backed MIDI data, decodes track events, and emits WinMM stream event records. |
| MIDI document release | omitted/helper-only | `0x005270e0-0x005271a3` | Releases parser/file/global-memory document state used by the MIDI stream. |
| SMF document load / stream fill helpers | omitted/helper-only | `0x005271b0-0x005277bb` | Loads `MThd`/`MTrk` track tables and fills WinMM stream buffers from parsed track events. |
| `g_pMidiPlayer` | generated global-data reference | `0x0067ab4c` | Active MIDI singleton pointer. |
| [UID:0000T9][MidiPlayerWinMMState](by-global/MidiPlayerWinMMState.md) | generated/raw globals | `0x0069b500+` state family | Stream handle, callback event, header buffers, stream state, volume, current path, parser state. |

## Ownership Reasoning

- `class_MidiPlayer.meta_wave3` imports from `MidiPlayer.cpp` and summarizes the class as a MIDI music playback controller.
...
- The recovered helper globals are used only by MIDI stream teardown/reset paths and should migrate with `MidiPlayer`, not generic audio or application code.
...
- Active `class_MidiPlayer.cpp` emits constructor, destructor, playback enable/disable, stop, and scalar deleting destructor methods.
- `ReleaseMidiBuffers_00525B10.meta_wave3` says the helper unprepares two `0x54`-byte MIDI header slots and frees non-null `lpData` blocks.
...
- Read-only Wave3 inspection on 2026-05-25 still reports 7 generated `MidiPlayer` methods and missing-ref/autograder attention around omitted helper materialization. The documentation split should therefore remain source-layout guidance, not a claim that active generated output has all private MIDI helpers materialized.
- 2026-05-26 recheck using current `simroot_v2` and IDA MCP only: active `.cpp` output still omits `0x00525920`, `0x00525be0`, `0x00526100`, `0x00526240`, and `0x005270e0`, while IDA callers still keep those bodies in the MIDI stream/open/callback/select/reset/document cleanup family.
...
- Final names for [UID:0000T9][MidiPlayerWinMMState](by-global/MidiPlayerWinMMState.md) globals before generated-code renaming.
