...
NexusTK/
  ...
  ui/
    ...
    core/
      ...
      Region.cpp
      ScreenPane.cpp
      CursorManager.cpp       # provisional; current evidence favors folding cursor helpers into ScreenPane.cpp
      ScreenDimmer.cpp
      ScreenFadeOut.cpp
      VideoPlayerPane.cpp
      DialogPane.cpp
  ...
  map/
    ...
    RainingLayerPane.cpp
    SnowingLayerPane.cpp
  ...
  util/
    ...
    WaitableTimer.cpp
    PoolAllocator.cpp
    ThreadSafeNodeList.cpp        # alias candidate only; see PoolAllocator.cpp
    TimerMgr.cpp
    StringUtil.cpp
    StringBase.cpp
    Encoder.cpp
...
- `MSGHandler` owns application message-handler swapping and dispatch forwarding at `0x00528d60-0x00528e55`.
...
- `ExceptionHandler.cpp` owns the top-level unhandled-exception-filter singleton, `g_pCrashTarget`, `TopLevelExceptionFilter`, `BCrash.nfo` writing, crash stack helpers, and the diagnostic text packet helper at `0x004ab740`.
...
- `CheatDetector.cpp` owns the startup-created `CheatDetector` singleton at `0x00483f00-0x0048402c`, including [UID:0001P5][0x0067ab3c-0x0067ab40.g_pCheatDetector](by-memory/0x0067ab3c-0x0067ab40.g_pCheatDetector.md), a timer-handler style constructor, the [UID:000109][0x00483fe0-0x0048402d.CheatDetectorVirtualClearAndDestructor](by-memory/0x00483fe0-0x0048402d.CheatDetectorVirtualClearAndDestructor.md), the two-slot [UID:0001YU][SecuritySingletonVtables](by-type/by-vtable/SecuritySingletonVtables.md), and singleton teardown.
- `VirusChecker.cpp` owns the retained AhnLab V3 scanner wrapper at `0x005c0460-0x005c0fe1`, including [UID:0001NV][0x005c05a0-0x005c0937.VirusCheckerV3LoadAndScan](by-memory/0x005c05a0-0x005c0937.VirusCheckerV3LoadAndScan.md), [UID:0001NW][0x005c0940-0x005c0fe1.VirusCheckerProcessTreeHelpers](by-memory/0x005c0940-0x005c0fe1.VirusCheckerProcessTreeHelpers.md), the [UID:0001WG][VirusCheckerProcessTreeLayout](by-type/by-struct/VirusCheckerProcessTreeLayout.md), [UID:0001Q3][0x0069bf94-0x0069bf98.g_pVirusChecker](by-memory/0x0069bf94-0x0069bf98.g_pVirusChecker.md), and scan-result shutdown behavior.
...
Rationale: IDA MCP confirms both compact ranges and the application constructor xref for `CheatDetector`. `VirusChecker` has no proven live constructor caller in the current IDA state, but its DLL names, imports, and process/module scanner behavior are cohesive enough to document as a retained security module instead of library/runtime support.
...
- `MetaTable`, the per-table payload/decode/tree object, including [UID:0001CC][0x00524870-0x00524c54.MetaTableMaterializeRows](by-memory/0x00524870-0x00524c54.MetaTableMaterializeRows.md), [UID:0001CD][0x00524d10-0x00525914.MetaTableRowTreeHelpers](by-memory/0x00524d10-0x00525914.MetaTableRowTreeHelpers.md), and [UID:0001V7][MetaTableRowNode](by-type/by-struct/MetaTableRowNode.md).
...
See [UID:0000MS][ProfileStorage](by-file/ProfileStorage.md), [UID:0001QL][client_profile_storage](by-meta/client_profile_storage.md), and [UID:0001VR][RegistryConfigUserProfileBlock](by-type/by-struct/RegistryConfigUserProfileBlock.md). It groups profile-file and selected-profile helpers:

- `LoadUserProfileData`, `ImportLegacyUserProfileData`, and `SaveUserSettings_50ABA0`
- `RefreshSelectedProfileData`, pending later split review
- the local `.usr` binary format and legacy `.cfg` migration overlay into the `RegistryConfig` user-profile slice

Rationale: shared `Documents\NexusTK\users` file family, `.usr` serializer/loader pairing, legacy `.cfg` migration, and selected-profile sidecar/JPF refresh behavior. This module should call into `config/RegistryConfig.cpp` for defaults and field layout rather than owning config classes.
...
- `ServerSelectPane` and `ServerSelectMenuItemList` own the pre-login server-selection dialog and `DLGSERV` list UI at `0x00573d20-0x005747de`, plus [UID:0000S8][g_pServerSelectPane](by-global/g_pServerSelectPane.md) and the selected-server helper pair at `0x00574510-0x0057465d`.
- `TerminalPane` and `TerminalSetupPane` own the pre-login terminal/modem connection pane, terminal setup dialog, [UID:0000SF][g_pTerminalPane](by-global/g_pTerminalPane.md), [UID:0001JC][0x0058b130-0x0058b44b.TerminalPaneStreamParser](by-memory/0x0058b130-0x0058b44b.TerminalPaneStreamParser.md), [UID:0001JD][0x0058b620-0x0058b64e.TerminalPaneReconnectLeaveCallback](by-memory/0x0058b620-0x0058b64e.TerminalPaneReconnectLeaveCallback.md), and [UID:000247][0x0058dc60-0x0058dca5.TerminalPaneScalarDeletingDestructor](by-memory/0x0058dc60-0x0058dca5.TerminalPaneScalarDeletingDestructor.md) at `0x0058af50-0x0058c350`.
- `PhoneBookDialog`, `PhoneBookListPane`, `PhoneEntryDialog`, and `DialDialog` own the terminal phone-book and modem dialing flow at `0x0058c350-0x0058dc5a`; [UID:0001JF][0x0058cfa0-0x0058cfdb.PhoneBookSelectedEntryHelper](by-memory/0x0058cfa0-0x0058cfdb.PhoneBookSelectedEntryHelper.md) is now documented as a phone-book selected-entry helper, [UID:0001JL][0x0058dcb0-0x0058dcd3.PhoneBookFormattedWriteHelper](by-memory/0x0058dcb0-0x0058dcd3.PhoneBookFormattedWriteHelper.md) remains a phone-book formatted-write helper under review, and [UID:0001JH][0x0058da20-0x0058db94.DialDialogModemResponseParser](by-memory/0x0058da20-0x0058db94.DialDialogModemResponseParser.md) is the dial modem-response parser. The final split from `TerminalPane.cpp` remains provisional.
...
Rationale: IDA confirms the compact `ConnStatusPane` block at `0x00494520-0x004949df` and reconnect dialog neighborhood at `0x00553c10-0x00554635`. A 2026-05-25 recheck shows this neighborhood contains an interleaved [UID:0000IH][CopyWindow](by-file/CopyWindow.md) constructor/callback island at `0x005544c0-0x0055454a`; keep that island with `ui/dialogs/CopyWindow.cpp`. The remaining connection/reconnect classes are UI classes, but their behavior is driven by network/session state, making `network/` a stronger initial placement than generic `ui/dialogs/`.

### `util/List.cpp`, `util/LinkedList.cpp`, `util/Queue.cpp`, `util/Deque.cpp`, template helpers, and utility containers
...
- `LinkedList` owns the sentinel-based intrusive-list support at `0x00457550`, `0x004570e0`, and `0x00457430`. It is shared by `FrameMgr`, `MiniMapVersionManager`, `MonsterImageLib`, and `DATIndexVector` teardown, so it should not be folded into any one feature module.
...
- `VectorHelpers.h` tracks generated vector support such as `UInt32Vector` and `RecordVector_4E57C0`; feature modules consume these helpers but should not absorb the generic vector machinery.
- `Deque.cpp` owns the non-contiguous 4-byte-entry block deque methods at `0x00439e00`, `0x0043e0b0`, `0x004a88e0`, `0x0054c0c0`, `0x0054c160`, and `0x0057b9e0`.
- `CachedHashTable.cpp` owns a fixed-entry hash/cache island at [UID:00016O][0x004c6160-0x004c659c.CachedHashTableLifecycle](by-memory/0x004c6160-0x004c659c.CachedHashTableLifecycle.md), including the helper island at [UID:00016P][0x004c62d0-0x004c64de.CachedHashTableRawHelpers](by-memory/0x004c62d0-0x004c64de.CachedHashTableRawHelpers.md), modeled [UID:00016Q][0x004c64b0-0x004c64c4.CachedHashTableCountAccessors](by-memory/0x004c64b0-0x004c64c4.CachedHashTableCountAccessors.md), primary vtable [UID:0001X6][CachedHashTableVtable](by-type/by-vtable/CachedHashTableVtable.md), and exact vtable-data child [UID:0002M7][0x0061aac4-0x0061aad0.CachedHashTableVtableData](by-memory/0x0061aac4-0x0061aad0.CachedHashTableVtableData.md); current IDA caller checks show no direct users, so treat it as a low-priority utility/legacy candidate.
- `PrimeNumberGenerator.cpp` owns the raw indexed-prime accessor at `0x0054bcc0`, the wheel-factored sieve utility at `0x0054bcf0`, scalar deleting destructor `0x0054c110`, and one-slot vtable [UID:0001YG][PrimeNumberGeneratorVtable](by-type/by-vtable/PrimeNumberGeneratorVtable.md); current IDA caller checks also show no direct users, so keep it as utility/legacy support rather than assigning it to a feature.

Rationale: IDA MCP caller checks show `List::List` fan-in from render/resource code, UI panes, object lists, map helpers, and other systems. `LinkedList` caller/xref checks span frame scheduling, minimap hashes, monster image archive-bound lists, and DATIndexVector list teardown. `Deque` caller checks span EventDispatcher, FolderTreePane, MapPane, WorldMapPane, and SoundManager, and the 2026-05-25 recheck identifies a non-emitted element-address helper used by `PrimeNumberGenerator`. `Queue` has a compact class/vtable cluster even though no direct constructor caller is currently modeled. `AUTOBUF`, `ProtectedArray`, and `VectorHelpers` emitted code appears as concrete template/helper instantiations, but the reusable behavior is shared container/header support while concrete records are owned by their feature modules. `CachedHashTable` and `PrimeNumberGenerator` have generic utility behavior and import-source provenance but no active caller evidence, making `util/` safer than feature ownership; the 2026-05-26 `CachedHashTable` pass adds a raw helper block and vtable that active `simroot_v2` missed, but still finds no feature-specific callers. `SortedList` and `HierList` have standalone RTTI/vtables and source-file provenance, but both are utility containers; 2026-05-25 IDA rechecks confirm their omitted helper functions are real and class-local despite active `simroot_v2` omissions. That breadth supports `util/List.cpp`, `util/LinkedList.cpp`, `util/Queue.cpp`, `util/Deque.cpp`, `util/AUTOBUF.h`, `util/ProtectedArray.h`, `util/VectorHelpers.h`, `util/CachedHashTable.cpp`, `util/PrimeNumberGenerator.cpp`, `util/SortedList.cpp`, and `util/HierList.cpp`, not `ui/`, `map/`, or `render/`.
...
- `FunctionObject` and `FunctionObject0` own base destructor/pure-virtual callback support; [UID:000123][0x0049b090-0x0049b28e.FunctionObjectDestructors](by-memory/0x0049b090-0x0049b28e.FunctionObjectDestructors.md) is the confirmed `FunctionObject` destructor support currently documented.
...
See [UID:0000JD][FileIO](by-file/FileIO.md), [UID:00004V][File](by-class/File.md), [UID:0000E6][StdioFile](by-class/StdioFile.md), [UID:0001XK][FileStreamVtables](by-type/by-vtable/FileStreamVtables.md), [UID:0001UG][FileStreamLayouts](by-type/by-struct/FileStreamLayouts.md), and [UID:0000V6][PathExistsViaStat_00582460](by-item/PathExistsViaStat_00582460.md). The archive-specific companion docs are indexed through [UID:0000IM][DATArchive](by-file/DATArchive.md), but DAT ownership stays in `archive/` rather than this utility layer. Current evidence supports a small utility file I/O layer:
...
- `PathUtil.cpp` or `StdioFile.cpp` owns [UID:0000V6][PathExistsViaStat_00582460](by-item/PathExistsViaStat_00582460.md), a shared `_wstat`-based wide-path probe at `0x00582460-0x00582494`.
...
Rationale: `Encoder` and `Decoder` share an 18-byte buffer/cursor/flags layout and adjacent two-slot vtables. The current generated `Decoder` output is incomplete and reports only a 4-byte base-like class, so source migration should use the codec docs before trusting active `class_Decoder.cpp`.

### `util/Thread.cpp`, `util/RingBuffer.cpp`, `util/Monitor.cpp`, `util/WaitableTimer.cpp`, `util/ThreadSafeNodeList.cpp`, and `util/PoolAllocator.cpp`

See [UID:0000OR][Thread](by-file/Thread.md), [UID:0000N8][RingBuffer](by-file/RingBuffer.md), [UID:0000LI][Monitor](by-file/Monitor.md), [UID:0000P7][WaitableTimer](by-file/WaitableTimer.md), [UID:0000OS][ThreadSafeNodeList](by-file/ThreadSafeNodeList.md), [UID:0000MM][PoolAllocator](by-file/PoolAllocator.md), and [UID:0001QT][client_threading](by-meta/client_threading.md). Current evidence supports a utility threading/synchronization and fixed-block allocation family:
...
- `Monitor.cpp` owns [UID:00008L][Monitor](by-class/Monitor.md), [UID:00008M][MonitorCondition](by-class/MonitorCondition.md), [UID:0000BX][RequestSyncGate](by-class/RequestSyncGate.md), and [UID:00003C][CriticalSectionLock](by-class/CriticalSectionLock.md).
...
- `PoolAllocator.cpp` owns the fixed-block allocator island at `0x004b13d0-0x004b1584`, the chunk helper at `0x00549bd0-0x00549bf1`, and the shared allocator type used by [UID:0000TI][PoolAllocatorStaticInstances](by-global/PoolAllocatorStaticInstances.md).
- Concrete pool globals should stay beside their consuming modules: object-pane pools with the corresponding `map/*ObjectPane.cpp` files, the `Motion` pool with animation/motion code, the `UserPane` pool with `ui/panels/UserPane.cpp`, and the string-buffer pools with `StringUtil.cpp` or a future `StringBase.cpp`.
...
- `ThreadSafeNodeList.cpp` should remain an alias candidate only for the lock-protected intrusive-list behavior at `0x004b14c0-0x004b14ef`; current IDA evidence favors `PoolAllocator::Free` for proven string-pool call sites.
...
- The block transforms, inverse MixColumns helper, key expansion helper, forward/inverse S-boxes, Rcon table, and `0xff`-initialized scratch state belong with the same module.
- IDA currently finds no direct xrefs or stored function pointers to the raw wrappers, so this may be dead linked-in support code or an indirectly reached path not represented in current analysis.
...
See [UID:0000OT][TimerMgr](by-file/TimerMgr.md), [UID:0000F1][TimerMgr](by-class/TimerMgr.md), [UID:0000F2][TimerMgrTimerQueue](by-class/TimerMgrTimerQueue.md), [UID:0000F0][TimerHandler](by-class/TimerHandler.md), and [UID:0001QU][client_timing](by-meta/client_timing.md). This remains a strong multi-class utility module:
...
- `BackPane` owns the root background pane created by `InitializeMainUiGraph`, its frame-resource load, global singleton pointer, [UID:000161][0x004b8e00-0x004b8e11.BackPaneGetBounds](by-memory/0x004b8e00-0x004b8e11.BackPaneGetBounds.md), help-popup placement helpers, and `FRAME.PAL` versus `NPAL5.PAL` paint branch.
...
Rationale: IDA MCP caller checks show constructor calls only from `MapPane::ChangeMap` and `MapPane::HandleEffectPacket`; helper callers are MapPane render, scroll, hit-test, object cleanup, and diagnostics paths. The 2026-05-25 recheck confirms `simroot_v2` still splits lifecycle into `class_ObjectList.cpp` and the early accessor subset into `class_MapPaneSpatialIndex.cpp`, so migration should use [UID:00023D][0x00530d00-0x00530ed9.ObjectListRemoveByObjectIdHelper](by-memory/0x00530d00-0x00530ed9.ObjectListRemoveByObjectIdHelper.md), [UID:00023E][0x00531480-0x00532530.ObjectListCategorizeLookupHelpers](by-memory/0x00531480-0x00532530.ObjectListCategorizeLookupHelpers.md), [UID:0001D3][0x00532530-0x00532f67.ObjectListAccessorsAndSweeps](by-memory/0x00532530-0x00532f67.ObjectListAccessorsAndSweeps.md), and [UID:00023F][0x00532f70-0x0053728e.ObjectListExtendedTypeLookupHelpers](by-memory/0x00532f70-0x0053728e.ObjectListExtendedTypeLookupHelpers.md) as the complete method inventory. That supports a `map/ObjectList.cpp` companion rather than a generic utility module or a standalone `MapPaneSpatialIndex.cpp`.

### `map/MapNamePane.cpp`, `map/MapRefreshDimmer.cpp`, and weather layer panes
...
- `WeatherLayerPane` owns the base weather overlay bounds/timer logic; `RainingLayerPane`, `SnowingLayerPane`, and `SwallowLayerPane` own concrete rain, snow, and sprite-flight overlays selected by `MapPane` weather setup.
- Weather static tables should migrate with the weather-layer source family or a nearby `WeatherTables.cpp` helper, not as separate arbitrary globals. Current exact range docs split them into [UID:0001ON][0x006702c8-0x00670784.RainCoordinateTables](by-memory/0x006702c8-0x00670784.RainCoordinateTables.md), [UID:0001OO][0x00670784-0x006707d0.SnowPatternByteTables](by-memory/0x00670784-0x006707d0.SnowPatternByteTables.md), and [UID:0001Q4][0x0069bf98-0x0069c044.SnowLookupDescriptorStorage](by-memory/0x0069bf98-0x0069c044.SnowLookupDescriptorStorage.md).
- [UID:0001O1][0x005c1bd0-0x005c2ab5.SnowingAndSwallowLayerPanes](by-memory/0x005c1bd0-0x005c2ab5.SnowingAndSwallowLayerPanes.md) share an interleaved local island: Snowing owns `0x005c1bd0-0x005c2227`, `0x005c29ec`, `0x005c29f7`, and `0x005c2a20`, while Swallow begins at `0x005c2230` and owns the `0x005c2a02`/`0x005c2a0d`/`0x005c2a60-0x005c2ab5` tail pieces.
- [UID:0001O3][0x005c2760-0x005c292e.SwallowLayerPaneRawFlightHelpers](by-memory/0x005c2760-0x005c292e.SwallowLayerPaneRawFlightHelpers.md) are unreferenced/no-function raw bodies in the weather layer neighborhood, not `BackPane` methods.

Rationale: IDA confirms `MapNamePane` owns the method ranges `0x005031f0-0x0050349e`, thunks at `0x0050380b` and `0x00503816`, and scalar deleting destructor `0x00503840-0x005038fd`; [UID:00008B][MiniMapButtonPane](by-class/MiniMapButtonPane.md) begins inside the older aggregate at `0x00503580`, so use the [UID:0001AL][0x005031f0-0x0050395f.MapNameAndMiniMapButtonPanes](by-memory/0x005031f0-0x0050395f.MapNameAndMiniMapButtonPanes.md) for per-method migration. IDA also confirms `MapRefreshDimmer` at `0x00514920-0x00514e5a` and the weather layer family at `0x005c12a0-0x005c2ab5`, including their small compiler/vtable thunk islands. `MapPane` weather setup at `0x0050db50` constructs the concrete weather panes, so their call-in and behavior are map-screen specific. Final migration may keep them as separate `.cpp` files or private helper classes beside `MapPane`, but they should remain under `map/`.
...
- `TreeStorage<FolderTreePane::TreeElem>`, `Tree<FolderTreePane::TreeElem>`, `TreeItor<FolderTreePane::TreeElem>`, and the 36-byte `FolderTreePane::TreeElem` record should be grouped with `FolderTreePane.cpp`, not emitted as standalone original source files.
...
- `VersatileAlertPane` is a generic callback-backed accept/cancel wrapper over `AlertPane`; keep the raw cleanup body at [UID:00012Y][0x004a06e0-0x004a0714.VersatileAlertPaneRawCleanup](by-memory/0x004a06e0-0x004a0714.VersatileAlertPaneRawCleanup.md) with this class unless later xrefs prove it is only suppressible unwind glue.
...
- Keep this separate from [UID:0000HE][AlertPanes](by-file/AlertPanes.md) because it is text-edit-specific, and separate from [UID:0000ON][TextEditPane](by-file/TextEditPane.md) because it has its own dialog vtable and Wave2 source-file evidence for `CopyWindow.cpp`.

Rationale: IDA MCP confirms the accept/cancel callbacks and vtable xrefs, shows the active text-edit global at `0x0069be00` is also used by `TextEditPane` construction/destruction, and raw disassembly confirms the constructor body at `0x005544c0-0x00554512`. The `0x00553e60` helper still proves live inline `CopyWindow` construction, but 2026-05-25 caller checks show it is owned by nearby connection/reconnect dialog initialization.
...
### `ui/dialogs/ItemDialogs.cpp`, `ui/dialogs/MyItemListPane.cpp`, `ui/dialogs/ItemMenuDialogs.cpp`, and `ui/dialogs/MixItemDialog.cpp`
...
- `MyItemListPane` owns the reusable player-inventory list picker used by add-item, mix-item, and clan-deposit flows. Keep it adjacent to item dialogs, not in generic `ListPane.cpp` or social/clan ownership.
...
- `ServerItemMenuDialog`, `ServerItemMenuItemList`, `ClientItemMenuDialog`, and `ClientItemMenuItemList` own item context menus over the shared [UID:0000L9][MerchantDialogPane](by-file/MerchantDialogPane.md) base.
...
Rationale: IDA MCP confirms the exchange/mix item dialog cluster at `0x004ae4c0-0x004b0b15`, `MyItemListPane` constructor/draw functions at `0x004aeb30` and `0x004aec90`, and item menu dialog/list cluster from `0x0051a520` through `0x0051ca3c`. `MyItemListPane` constructor callers span `ClanDepItemDialog`, `AddItemDialog`, and `MixItemDialog`, which supports an item-picker support source rather than clan-only ownership. The 2026-05-26 vtable pass confirms item-dialog table bases from `0x00619e8c` through `0x0061a1b4` and the `MixItemDialog` callback table at `0x0061a3b0`; these tables are adjacent to unrelated RTTI/table data, so source migration should use the exact bases instead of broad `.rdata` neighborhoods. `AddEmployeeItemDialog::AddEmployeeItemDialog` at `0x004a4ae0` is a Wave3 projected boundary that IDA does not recognize as a function, and the 2026-05-24 employee pass shows the actual construction path lives in `EmployeeDialogPane::OnButtonAction`; keep the base picker here and migrate the employee-specific subclass with `EmployeeDialogPane.cpp`.
...
- `TextDialog` owns the `0x00552110-0x00553609` plain text/input dialog core: packet parsing, normal/high-resolution `DLGMSG` layouts, optional object art, text edit submission, and region-selection replies.
...
- `PostInputPane` owns a post-selection prompt and opcode `0x34`.
- `SpellSpellInputPane` owns spell-to-spell selection prompt behavior and may move to a later spell-input source.
- `SelfSaveInputPane` owns a one-character self-save confirmation prompt and opcode `0x25`; review it as a command-input family member, with constructor/factory boundary caveats from [UID:0000NM][SelfSaveInputPane](by-file/SelfSaveInputPane.md) and shared sender state via [UID:0000Q5][g_packetSender](by-global/g_packetSender.md).
...
- Keep the current `FolderTreePane` chat-message subset under review: `AddChattingMessage`, `AddIncomingMessage`, `SetVisibleRowCount`, and `HandleChatPacketType10` are chat-specific in behavior, but current Wave3/Wave2 ownership still attaches them to `FolderTreePane`. The filesystem tree-control portion is now documented separately in [UID:0000JG][FolderTreePane](by-file/FolderTreePane.md).
...
- `ClanStatusPane` owns the visible clan status panel, `CLAN*.EPF` painting, keyboard/mouse tab handling, row geometry, row drawing, and owned child list panes.
...
- `ClanEnlistInputDialog` owns clan attribute/list/member packet parsing and the enlistment submit path.
...
Rationale: IDA MCP confirms constructor, OK handler, focus handler, thunk, and destructor starts at `0x0053f2c0`, `0x0053f830`, `0x0053f930`, `0x005425b5`, `0x005425c0`, and `0x005426e0`. The class is small and feature-specific enough that it likely lived in its own dialog file or a social dialogs file, not in generic `DialogPane.cpp`. A 2026-05-25 send-helper pass also shows `OnOkButton` calls [UID:0001HX][0x00574e50-0x0057536b.BuildAndSendInventoryData](by-memory/0x00574e50-0x0057536b.BuildAndSendInventoryData.md), which reads the same 20 config friend-name slots and serializes opcode `0x77`; [UID:0000UP][FriendNameListSyncOpcodes](by-item/FriendNameListSyncOpcodes.md) identifies the other trigger as `MapPane` inbound case `0x6a`. Keep the editor in `social/FriendListDialog.cpp` while leaving the queued-send helper in the request-sender cluster until that interface is resolved.

### `audio/SoundManager.cpp` and `audio/MidiPlayer.cpp`
...
Keep [UID:0000NA][ScreenDimmer](by-file/ScreenDimmer.md) separate from `render/Effects.cpp` for now. `ScreenDimmer` and `ScreenFadeOut` form a generic UI overlay pane island at `0x00559b90-0x0055a252`, immediately before but not part of the `ScreenEffecter` runtime effecter cluster.
...
- `ItemObjImageLib` owns `ITEM.TBL`/`ITEM.TBD`, item draw variants, `ITEM.EPF`/`ITEM.EPD` frame lookup, [UID:0000RA][g_pItemObjImageLib](by-global/g_pItemObjImageLib.md), [UID:0001XW][ItemObjImageLibVtable](by-type/by-vtable/ItemObjImageLibVtable.md), [UID:0001UU][ItemObjImageLibLayout](by-type/by-struct/ItemObjImageLibLayout.md), [UID:0001UT][ItemInfo](by-type/by-struct/ItemInfo.md), [UID:00017O][0x004dee20-0x004dee4d.ItemObjImageLibDestructor](by-memory/0x004dee20-0x004dee4d.ItemObjImageLibDestructor.md), and likely the shared [UID:0000UQ][GetItemGlyphBounds_004DF460](by-item/GetItemGlyphBounds_004DF460.md) helper or a neighboring item-image helper file.
...
- [UID:0000LK][MonsterImageLibTables](by-file/MonsterImageLibTables.md) documents two small helper classes shared by the monster and riding parsers; keep their final placement open until the helper names are neutralized.
...
- `BA0`/`BA1` DAT-backed font archives loaded lazily through `EnsureFontSlotLoaded`.
...
- `DIBitmap` destructor/accessor helpers at `0x004a1740-0x004a17ac`, including the raw non-deleting destructor body and guarded width/height accessors currently omitted from active generated source.
...
See [UID:0000PC][Zlib](by-file/Zlib.md), [UID:0001QE][client_libraries](by-meta/client_libraries.md), [UID:0000W3][0x004142c0-0x00419e56.ZlibCore](by-memory/0x004142c0-0x00419e56.ZlibCore.md), [UID:0000W2][0x004142c0-0x00414565.ZlibPublicCompressChecksum](by-memory/0x004142c0-0x00414565.ZlibPublicCompressChecksum.md), [UID:0000W4][0x00414820-0x00417b97.ZlibDeflateAndTrees](by-memory/0x00414820-0x00417b97.ZlibDeflateAndTrees.md), [UID:0000W5][0x004158b0-0x0041600e.ZlibInflateApiAllocatorAndAdler](by-memory/0x004158b0-0x0041600e.ZlibInflateApiAllocatorAndAdler.md), and [UID:0000W6][0x00417ba0-0x00419e56.ZlibInflateInternals](by-memory/0x00417ba0-0x00419e56.ZlibInflateInternals.md). This third-party support folder should own:
...
- deflate setup/reset and `deflate.c` internal helpers from `0x00414570-0x004158aa`;
- deflate tree/bit-output helpers from `0x00416010-0x00417b97`;
- inflate setup/reset/blocks/codes/tree/fast helpers from `0x004158b0-0x00415e77` and `0x00417ba0-0x00419e56`.

Rationale: IDA strings identify zlib 1.1.4 directly with deflate/inflate copyright text, and source comparison against staged official `zlib-1.1.4` now maps the binary to the stock source files (`compress.c`, `uncompr.c`, `crc32.c`, `deflate.c`, `inflate.c`, `zutil.c`, `adler32.c`, `trees.c`, `infblock.c`, `infcodes.c`, `inftrees.c`, `infutil.c`, and `inffast.c`). `MetaTable`, palette, startup-logo, and image-resource code consume zlib; they should not own zlib helpers.

### `third_party/libjpeg/`

See [UID:0000KN][LibJPEG](by-file/LibJPEG.md), [UID:0000VP][0x00401000-0x00414283.LibJpeg6bCore](by-memory/0x00401000-0x00414283.LibJpeg6bCore.md), [UID:0000VR][0x00401b00-0x004038c7.LibJpegMarkerReader](by-memory/0x00401b00-0x004038c7.LibJpegMarkerReader.md), [UID:0000VV][0x00404a60-0x0040505b.LibJpegInputController](by-memory/0x00404a60-0x0040505b.LibJpegInputController.md), [UID:0000VX][0x00405c80-0x004063c8.LibJpegDecompressMaster](by-memory/0x00405c80-0x004063c8.LibJpegDecompressMaster.md), [UID:0000VZ][0x004064a0-0x00406d8c.LibJpegMarkerWriter](by-memory/0x004064a0-0x00406d8c.LibJpegMarkerWriter.md), and [UID:0001QE][client_libraries](by-meta/client_libraries.md). This third-party folder should own the statically linked IJG libjpeg 6b code currently bounded at `0x00401000-0x00414283`.

Rationale: IDA strings identify IJG libjpeg directly through `Copyright (C) 1998, Thomas G. Lane`, `6b  27-Mar-1998`, and canonical JPEG diagnostics such as `Wrong JPEG library version`, `Not a JPEG file`, `Invalid JPEG file structure`, and `JPEGMEM`. IDA also shows `0x00401000` initializing `off_631660`, a 121-entry JPEG message table at `0x00631660-0x00631840`, while `0x00405060` reads `JPEGMEM` for the JPEG memory manager. Product wrappers now include [UID:000175][0x004d05f0-0x004d0c57.ImageDecodeWrappers](by-memory/0x004d05f0-0x004d0c57.ImageDecodeWrappers.md) and [UID:00018N][0x004efbc0-0x004efe94.JpegScreenshotWriter](by-memory/0x004efbc0-0x004efe94.JpegScreenshotWriter.md), but IJG internals should not be assigned to render feature files.

Rebuild note: obtain official `jpegsrc.v6b` source and compare stock strings/control flow before replacing generated helper files. Current source anchors now cover public decompress/compress API files, `jdmarker.c`, `jcomapi.c`, `jdinput.c`, `jdmaster.c`, `jutils.c`, `jcmarker.c`, `jcinit.c`, `jmemmgr.c`, and `jmemnobs.c`; remaining work is mostly entropy, coefficient, color, sampling, quantizer, and DCT/IDCT lower-helper mapping. Exclude `0x00414290-0x004142b5` because it is Miles `_AIL_shutdown@0` thunk/atexit glue, and treat `0x004142c0+` as [UID:0000W3][0x004142c0-0x00419e56.ZlibCore](by-memory/0x004142c0-0x00419e56.ZlibCore.md). The earlier `0x00469640-0x00470dc9` hypothesis is rejected because IDA decompiles that neighborhood as BrowserWindow/WebBrowser COM code.

### `third_party/jsoncpp/`

See [UID:0000KI][JsonCpp](by-file/JsonCpp.md), [UID:0000WY][0x00424630-0x00428b4b.JsonCppValueCore](by-memory/0x00424630-0x00428b4b.JsonCppValueCore.md), [UID:0000WZ][0x00428b4b-0x004298f0.JsonCppValuePathIteratorAndCrtNumericSupport](by-memory/0x00428b4b-0x004298f0.JsonCppValuePathIteratorAndCrtNumericSupport.md), [UID:0000X0][0x004298f0-0x0042acdf.JsonCppDocumentParseFrontEnds](by-memory/0x004298f0-0x0042acdf.JsonCppDocumentParseFrontEnds.md), [UID:0000X1][0x0042acdf-0x0042ae90.JsonCppLegacyReaderRawLexicalHelpers](by-memory/0x0042acdf-0x0042ae90.JsonCppLegacyReaderRawLexicalHelpers.md), [UID:0000X7][0x00431d50-0x00431f1c.JsonCppOurReaderParse](by-memory/0x00431d50-0x00431f1c.JsonCppOurReaderParse.md), [UID:0001ZR][0x00431f20-0x00432e10.JsonCppOurReaderErrorTokenHelpers](by-memory/0x00431f20-0x00432e10.JsonCppOurReaderErrorTokenHelpers.md), [UID:0000X2][0x0042ae90-0x0042edeb.JsonCppReaderBuilderIsland](by-memory/0x0042ae90-0x0042edeb.JsonCppReaderBuilderIsland.md), [UID:0000X8][0x00432e10-0x004345ee.JsonCppSecondParserIsland](by-memory/0x00432e10-0x004345ee.JsonCppSecondParserIsland.md), [UID:0000X9][0x004345f0-0x00434887.JsonCppNumberTokenParseHelper](by-memory/0x004345f0-0x00434887.JsonCppNumberTokenParseHelper.md), [UID:0001ZS][0x00434890-0x004361ae.JsonCppOurReaderUnicodeAndBuiltStyledStreamWriter](by-memory/0x00434890-0x004361ae.JsonCppOurReaderUnicodeAndBuiltStyledStreamWriter.md), [UID:0000X3][0x0042e850-0x0042f3f0.JsonCppStreamWriterBuilderIsland](by-memory/0x0042e850-0x0042f3f0.JsonCppStreamWriterBuilderIsland.md), [UID:0000X4][0x0042f3f0-0x00430040.JsonCppLegacyWriterCore](by-memory/0x0042f3f0-0x00430040.JsonCppLegacyWriterCore.md), [UID:0000X5][0x00430040-0x00430092.GenericStringAppendHelper](by-memory/0x00430040-0x00430092.GenericStringAppendHelper.md), [UID:0000X6][0x00430092-0x00431d50.JsonCppWriterTailAndOurReaderPrelude](by-memory/0x00430092-0x00431d50.JsonCppWriterTailAndOurReaderPrelude.md), [UID:0000XC][0x004417e0-0x00441d24.JsonCppValidReaderKeys](by-memory/0x004417e0-0x00441d24.JsonCppValidReaderKeys.md), [UID:0000XD][0x00441d30-0x00442084.JsonCppValidWriterKeys](by-memory/0x00441d30-0x00442084.JsonCppValidWriterKeys.md), and [UID:0001QE][client_libraries](by-meta/client_libraries.md). This third-party folder should own the statically linked JsonCpp parser/writer implementation from the current best upstream commit candidate.
...
- the `0x0069b3e0-0x0069b410` render callback table and generated aliases such as `g_pfnLockSurface` / `dword_69B3FC`;
...
Rationale: current owners such as `FolderTreePane`, `RankingUserListPane`, `FittingRoomDownloadControlPane`, and `MapPane` describe observed call paths, but many helpers are generic DirectDraw/surface utilities. A 2026-05-25 IDA pass also shows `0x00559410` invoking [UID:0000MC][Pane](by-file/Pane.md) dirty/motion helpers, reinforcing that nearby presentation traversal is shared UI/render code rather than map gameplay source. `Surface.cpp` keeps those functions visible for source migration without prematurely folding them into one feature module.

### `ui/core/Pane.cpp`, `ui/core/Layer.cpp`, `ui/core/RectBounds.cpp`, `ui/core/Region.cpp`, `ui/core/DialogPane.cpp`, `ui/core/ControlPane.cpp`, `ui/core/ScreenPane.cpp`, `ui/core/CursorManager.cpp`, and screen overlay helpers
...
- `RectBounds` owns the lower-level four-int geometry value object and point/rectangle free-helper island at `0x004b78d0-0x004b7f87`; keep it separate from `Region` until original-source evidence proves a combined geometry file. The related [UID:00016H][0x004bb2e0-0x004bb5a5.LineClipHelpers](by-memory/0x004bb2e0-0x004bb5a5.LineClipHelpers.md) should remain in the RectBounds/render-geometry review bucket until final callback-source ownership is proven.
...
- `CursorManager` is a provisional screen/cursor companion source, not a proven standalone manager. Current generated code calls `0x0067a7cc` [UID:0000QP][g_pCursorManager](by-global/g_pCursorManager.md), but IDA confirms `ScreenPane::ScreenPane` writes the same storage as the root `g_pScreenPane`. Current migration should prefer folding these helpers into `ScreenPane.cpp`; keep a small adjacent `CursorManager.cpp` only if later source-layout evidence proves an original split.

2026-05-26 recheck: current active `simroot_v2/class_CursorManager.cpp` still emits a standalone generated view with repeated synthetic `CursorManagerLayout` declarations, while IDA MCP reports no separate cursor-manager lifetime and confirms `0x0067a7cc` as one 4-byte `.data` pointer with 145 refs. A follow-up IDA decompile pass shows `ScreenPane::ScreenPane` initializes the root layer list at `+0x10c`, dimensions at `+0x112/+0x114/+0x116`, and frame/fade fields, but not the cursor handle table used at `+0x550`; the missing cursor setup path should be researched before declaring a separate file.
- [UID:0000NA][ScreenDimmer](by-file/ScreenDimmer.md) owns generic modal/transition overlays: `ScreenDimmer`, `ScreenFadeOut`, [UID:0000S5][g_pScreenDimmer](by-global/g_pScreenDimmer.md), and [UID:0000PJ][CreateScreenDimmer_4A12B0](by-global/CreateScreenDimmer_4A12B0.md). Keep [UID:0000L4][MapRefreshDimmer](by-file/MapRefreshDimmer.md) under `map/` as the timer-backed map-refresh wrapper.
- `TextButtonControlPane` is a controls module, but its `DrawTextInRect` helper is broad shared UI text-rendering support and may move to a generic text helper if later evidence justifies it.
...
- The tab-switch path calls [UID:0000YI][0x004615b0-0x00461657.AboveFrameRefreshHelpers](by-memory/0x004615b0-0x00461657.AboveFrameRefreshHelpers.md), but those helpers remain [UID:0000JL][FrameChrome](by-file/FrameChrome.md) dependencies because they release/rebuild the `0x0067a840` frame slot.
...
- `SoundStatusPane` owns the compact always-present sound/music status caption and hotkey surface.
- It is constructed by `InitializeMainUiGraph` in both the newer and older layouts at `0x004f8460` and `0x004f89c7`.
- The pane depends on [UID:0000NV][SoundManager](by-file/SoundManager.md), `g_pMidiPlayer`, and config offset `g_pConfig + 0x28de48`, but should remain under `ui/panels/` because it owns UI event handling, repaint, and caption formatting rather than playback implementation.
...
Rationale: IDA confirms the compact SoundStatusPane island at `0x0057bf70-0x0057c2cf`, with constructor, cleanup helper, hotkey handler, caption refresh, singleton clear helper, destructor adjustor thunks, and scalar deleting destructor. The active generated output omits `0x0057c050` and `0x0057c240`, and disabled output mishandles `0x0057c24b`/`0x0057c256`, so migration should use the new class/file/memory/global docs as the boundary anchor.
...
- [UID:0000YI][0x004615b0-0x00461657.AboveFrameRefreshHelpers](by-memory/0x004615b0-0x00461657.AboveFrameRefreshHelpers.md) are called from [UID:0000JQ][GeneralPurposePanel](by-file/GeneralPurposePanel.md) tab switches, but source ownership stays with frame chrome because they release/rebuild `AboveFrame` slot `0x0067a840`.
...
- `ScrollBar.cpp` owns `ScrollPane`, `ScrollWidget`, `ScrollablePane`, scrollbar size tables, and likely `CScrollBarBackPane`.
- 2026-06-01 IDA MCP sync: [UID:0000NF][ScrollBar](by-file/ScrollBar.md) now has validator path `NexusTK/ui/core/`; `CScrollBarBackPane` child docs attach to it after current evidence confirmed constructor bytes `0x0053fea0-0x0053fed2`, cleanup helper `0x0053fee0-0x0053feff`, draw method `0x0053ff00-0x0053ff8d`, destructor `0x00542680-0x005426d5`, vtable refs, and padding.
...
- `SimpleListPane` owns a small bounds-driven `ListPane` wrapper at raw constructor range `0x005739a0` and destructor/thunks at `0x00573c38-0x00573d15`; use a separate `SimpleListPane.cpp` unless final migration chooses to fold tiny control adapters into `ListPane.cpp`.
...
Rationale: IDA MCP confirms broad helper caller fanout for `0x004f3bd0`, `0x004f3c50`, `0x004f3dc0`, `0x004f3e20`, `0x004f3e80`, and `0x004f3f00`, crossing many independent UI features. IDA also confirms `SimpleListPane` destructor thunks/body at `0x00573c38`, `0x00573c43`, and `0x00573c50`, while the constructor at `0x005739a0` remains raw/non-IDA. That breadth matches a reusable control source, while `ScrollablePane` remains in `ScrollBar.cpp` as scrollbar/viewport infrastructure.
...
- `TextEditPane.cpp` owns `TextEditPane`, `TextEditScrap`, `TextEditObject`, `EPFTextEditObject`, `TextRunMeasureCallback`, `DrawTextRunCallback`, and tightly coupled text-edit helpers.
...
### `ui/controls/StaticTextControlPane.cpp`, `ui/controls/FontStyle.cpp`, `ui/controls/ScrolledTextControlPane.cpp`, `ui/controls/ScrolledPictureControlPane.cpp`, and `ui/controls/TextBoxPane.cpp`

See [UID:0000O8][StaticTextControlPane](by-file/StaticTextControlPane.md), [UID:0000JI][FontStyle](by-file/FontStyle.md), [UID:0000NI][ScrolledTextControlPane](by-file/ScrolledTextControlPane.md), [UID:0000NH][ScrolledPictureControlPane](by-file/ScrolledPictureControlPane.md), and [UID:0000OJ][TextBoxPane](by-file/TextBoxPane.md). Current evidence supports neighboring static/scrolled control modules:
...
- `FontStyle.cpp` owns a compact text-style descriptor at `0x00499f10-0x00499fda`, with cleanup at `0x004536e0-0x00453732`; it is consumed by minimap labels, mail compose labels, static text controls, and `HelpPane` style-copy setup.
...
- `TextBoxPane.cpp` owns the small `TextEditPane` derivative that customizes cursor visibility and line invalidation. It can remain separate for now, but later may fold into `TextEditPane.cpp` if the original source used one text-edit implementation file.

Rationale: the static text classes are tightly adjacent around `0x00498dd0-0x0049b925`, with text-effect helpers at `0x004bafa0-0x004bb0da`. `FontStyle` is small and partly discontiguous, but IDA caller evidence ties it to the same text-control/help-pane style pipeline rather than to minimap or mail feature code. The scrolled text classes are parallel around `0x004ff040-0x00502cbd`, while the picture scroller starts immediately afterward at `0x004ff7d0` and shares the same timer/control shape but not the text-rendering pipeline. A 2026-05-25 IDA recheck still finds no direct constructor caller for `0x004ff7d0`; generated fitting-room/head-selection labels remain owner pollution until a factory/config construction path is found. `TextBoxPane` has eight direct constructor callers and a distinct class identity while still depending on the text-edit core.
...
See [UID:0000HY][ButtonControlPane](by-file/ButtonControlPane.md), [UID:0000MP][PrettyButtonControlPane](by-file/PrettyButtonControlPane.md), and [UID:0000NY][SpecializedButtonPanes](by-file/SpecializedButtonPanes.md). Current evidence supports a generic button/choice-control module plus smaller specialized button modules:

- `ButtonControlPane.cpp` likely owns `ButtonControlPane`, `ImageButtonControlPane`, `TextButtonExControlPane`, `CheckBoxControlPane`, `RadioGroupControlPane`, `SliderControlPane`, `SelectBoxControlPane`, and `SelectionControlPane`, or at least the shared generic-control subset of those classes.
...
- `SpecializedButtonPanes.cpp` is a provisional bucket for `DirectionButtonControlPane`, `GenderButtonControlPane`, and `CheckBoxTextControlPane`. `ObjectImageButtonPane` was previously listed there, but current evidence supports moving it to the object-image control source family. `DirectionButtonControlPane` and `GenderButtonControlPane` have IDA-confirmed virtual methods/destructors, but their Wave3 constructor starts are not IDA function starts; child setup is currently observed inline in `NewUserMiscDialogPane`. See [UID:0001W7][SpecializedButtonPaneLayouts](by-type/by-struct/SpecializedButtonPaneLayouts.md) and [UID:0001YW][SpecializedButtonPaneVtables](by-type/by-vtable/SpecializedButtonPaneVtables.md) for the verified field/vtable evidence.
- `CheckBoxControlPane` remains a generic choice-control member of this family even though it has its own split-candidate page: IDA confirms consumers from both minimap and mail, and its layout/vtables now show a normal reusable control object rather than a feature-local dialog helper.

Rationale: the button/choice controls share `ControlPane`-derived construction, vtable-adjustor/destructor neighborhoods, and EPF-backed state painting. Generated helper names in this area are known to be polluted by fitting-room, bow-gauge, and base-pane names, so file ownership should be based on xrefs, address locality, and control behavior rather than isolated generated owner labels.
...
- `RectangleControlPane` owns solid-rectangle control behavior at `0x00499c60-0x00499d36`, plus destructor/thunk support and fixed type id `21`.
...
- Keep both classes under `ui/controls` unless later evidence proves the original project folded these tiny visual controls into a broader `BasicControlPanes.cpp` file.
...
See [UID:0000IP][DATIndexVector](by-file/DATIndexVector.md). This standalone helper should own:
...
- `archive/`: the core split is now documented as [UID:0000IN][DATFile](by-file/DATFile.md), [UID:0000IO][DATFileMgr](by-file/DATFileMgr.md), and [UID:0000IP][DATIndexVector](by-file/DATIndexVector.md). [UID:0000TH][ParseEntries_004A5E60](by-global/ParseEntries_004A5E60.md) is resolved as `DATFile.cpp` private/static-style support after the 2026-05-25 IDA MCP recheck. [UID:0000T5][LoadIndexedDATSeries](by-global/LoadIndexedDATSeries.md) is assigned to `app/Application.cpp` startup policy, while [UID:00012C][0x0049be70-0x0049be7c.ForwardLoadDATFileIndex](by-memory/0x0049be70-0x0049be7c.ForwardLoadDATFileIndex.md) remains archive-manager API. EPF/image frame helpers should be treated as render-resource code unless later evidence proves an archive owner. Loose metadata cache work belongs to [UID:0000LC][MetaMan](by-file/MetaMan.md), not archive. Generic list-node allocation and cleanup now belong to [UID:0000KR][LinkedList](by-file/LinkedList.md); only DATIndexVector's bucket-aware keyed helper stays in archive.
...
- `audio/`: remaining work is mostly ownership cleanup around `SoundManager` globals, MIDI stream globals, and [UID:0000DI][SoundPathVector](by-class/SoundPathVector.md) method boundaries. The main `SoundManager` and [UID:0000LD][MidiPlayer](by-file/MidiPlayer.md) files are now documented, and the former timer-wrapper question is resolved to [UID:0000OT][TimerMgr](by-file/TimerMgr.md). UI audio consumers such as [UID:0000NX][SoundStatusPane](by-file/SoundStatusPane.md) and [UID:0000LN][MusicControlDialog](by-file/MusicControlDialog.md) should stay in UI folders while calling into audio modules.
...
- `network/`: remaining protocol helper clustering after the now documented `Socket`, `PacketBuffer`, `FileDownloader`, and `CashShopRequest` send-queue pass. [UID:0000LG][MiscWorkThread](by-file/MiscWorkThread.md) uses WinInet but is currently placed under `app/` because it is application-owned and multiplexes non-network shell work too. `ProtocolSend.cpp` and `Session.cpp` remain placeholders until caller clusters prove them; `g_packetSender` lifetime should stay with `Socket.cpp`, and `0x00596960` should stay with [UID:0000OR][Thread](by-file/Thread.md), while the queue/send interface is reviewed.
...
1. Continue archive/resource module docs around the remaining image-loader/surface helpers and palette helpers; `ResourceLayoutStore` has been resolved as a `List`-backed alias, and `DATIndexVector`'s `0x00457310` helper is now documented as private EH cleanup support pending Wave3 ownership dry-runs.
