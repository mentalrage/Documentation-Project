## Agent C003 Specific Instructions ##
- Assigned Agent ID: C003
- Use the target-selection guidance below. Do not rely on only one queue or report.

## Required Status Reporting ##
- Follow `tools\leaser\status\README.md`. The supervisor owns registration and online/offline state; use only `python .\tools\leaser\status\status.py -report STATUS C003 "actionable details"` from `source-3\project-documentation`.
...
## C-Agent IDA Update Workflow ##
C agents follow the same documentation, leasing, target-selection, by-structure, scoring, and quality rules as A agents, with one additional capability: C agents may also make targeted updates inside IDA when the documentation work exposes a verified fix. The current IDA workflow is IDA 9.1; do not assume IDA 9.2 C++ parser features are available.

Use IDA as an evidence and verification tool together with the existing documentation. Verify candidate fixes against IDA MCP evidence and the relevant by-* documents before applying them. Safe IDA fixes, such as clearly verified function/global/type/member names, may be applied when confidence is extreme and the change is low risk. More invasive IDA changes, including class layouts, prototypes, vtables, structure definitions, calling conventions, or broad type imports, should wait until the related documentation has extreme confidence and the item has enough evidence to defend the change.

Do not make speculative IDA edits just because a name, type, or parent seems plausible. If an IDA update is useful but not yet safe, improve the documentation first and record the remaining evidence needed. IDA changes should support the same end goal as the documentation: the most accurate possible reconstruction of the original C++ source, not cleaner-looking decompiler output at the cost of correctness.
...
6. `E:\NTK\GhidraBridge\source-3\project-documentation\inference_research.md` For information on how to properly infer things while reverse engineering
...
3. Direct manual edits are limited to files inside `E:\NTK\GhidraBridge\source-3\project-documentation\by-*` folders and files inside your own `E:\NTK\GhidraBridge\source-3\project-documentation\tools\leaser\Agents\Agent-C003` folder.
...
4. For reconstructable items, do not attach children to parent items unless both the child and the actual direct parent have at least 85 completion and 85 confidence, with the scores justified by evidence in the documentation.
