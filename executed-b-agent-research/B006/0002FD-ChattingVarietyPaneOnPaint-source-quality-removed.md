...
## Removed operational snapshot from 1AA0EE1B6F645DC739922C504BDB6BEA331137F8E34E3636808D0ECC8157F075

> Source: [0002FD-ChattingVarietyPaneOnPaint-source-quality.md](./0002FD-ChattingVarietyPaneOnPaint-source-quality.md)
...
B006 performed read-only research. No IDA mutation, save, backup, worker/process operation, or report-lifecycle action is claimed. The primary supervisor owns Gate 2B and must apply or reject each accepted row directly through the live MCP, then independently reread the literal post-state. The report contains no transaction program, process launcher, hostile suite, or executable fixture.

### Structured Supervisor IDA Handoff

### Literal Current MCP Endpoint And Request Binding

The structured rows below are bound to the current installed MCP modification implementation, not to generic mutation verbs. The inspected authorities are `api_modify.py` SHA256 `161B314A14DAAA34C718176444292597BBB7498FA80158E25C358542BC16D1B8` / 51,759 bytes and `api_types.py` SHA256 `A3E7444BBAC0066B48EDF38E26F34E6ABEC823585CD50E43C6DA98C55B965ECF` / 37,540 bytes. Immediately before Gate 2B, the primary supervisor must confirm that the live `tools/list` still exposes the literal endpoints and fields used here. Schema drift, a missing narrow endpoint, a different canonical path, a pre-state mismatch, or an ambiguous/pre-existing returned worker stops before mutation and returns the row for renewed review.

- If a new session is needed, the only authorized open shape is `idb_open {input_path:"E:\\NTK\\Resources\\NexusTK\\NexusTK.exe.i64",mode:"force_headless",run_auto_analysis:false,build_caches:false,init_hexrays:false,idle_ttl_sec:3600,preferred_session_id:"<fresh-unique-supervisor-UID0002FD-role>"}`. The supervisor must bind `<active-session-id-from-current-idb_open>` only from that call's returned `session.session_id`; `idb_list` is corroboration, not replacement authority. Every request below uses that exact returned database value. No open in this handoff may omit `run_auto_analysis:false` or substitute `true`.
- Each function-name dry run and actual write uses only `rename` with a one-item `func` object and literal `pure:true`: A01 dry run is `rename {batch:{func:{addr:"0x00480e80",name:"ChattingVarietyPane__OnPaint"},stop_on_error:true,dry_run:true,allow_overwrite:false,pure:true},database:"<active-session-id-from-current-idb_open>"}` and the actual request is identical except `dry_run:false`; A02 uses the same two requests with `addr:"0x004b9670"` and `name:"GrafPort_SetTextBackFillColor"`. Each response must contain exactly one successful `func` result and `summary.total:1`, `ok:1`, `failed:0`, `stopped:false`; the dry response must also report `dry_run:true`. Before the A01 dry run and after both A01 rename calls, complete xref/caller readback must show exactly the sole inbound data xref `0x00614ee4 -> 0x00480e80` and zero ordinary code callers. Before the A02 dry run and after both A02 rename calls, complete xref readback must equal exactly the 11 inbound addresses `0x00467be6`, `0x00467e26`, `0x00467ec4`, `0x00480ead`, `0x0048216f`, `0x004b1c7b`, `0x00550e87`, `0x00550fe7`, `0x0057c1a5`, `0x00585bf5`, and `0x005bc820`. Any `error`, `dir`, `dir_error`, overwrite, `/vibe/` placement, decompiler refresh, type/frame/comment/body/xref/caller delta during the dry run or rename-only step fails the action.
- A01 physical-frame renames use the current specialized `rename.batch.stack` path, one row and one dry/actual pair at a time, always with `pure:false`: `{func_addr:"0x00480e80",old:"var_130",new:"thisPane"}`, then `{func_addr:"0x00480e80",old:"outContext",new:"tileContext"}`, then `{func_addr:"0x00480e80",old:"Destination",new:"modeText"}`. Each full request is `rename {batch:{stack:<literal-row>,stop_on_error:true,dry_run:<true-then-false>,allow_overwrite:false,pure:false},database:"<active-session-id-from-current-idb_open>"}`. A02 uses the same endpoint and fields with `{func_addr:"0x004b9670",old:"arg_0",new:"color"}`. `rename.batch.local` is forbidden because these are occupied physical frame members, not Hex-Rays-only locals. After every dry run and actual write, `stack_frame {addrs:"<function-address>",database:"<active-session-id-from-current-idb_open>"}` must reread the complete ordered frame; only the named row may change on the actual call.
- Physical frame-member types use only `set_type` with `kind:"stack"` after the successful physical rename: `set_type {edits:{addr:"0x00480e80",kind:"stack",name:"thisPane",ty:"ChattingVarietyPane *"},database:"<active-session-id-from-current-idb_open>"}`, `set_type {edits:{addr:"0x00480e80",kind:"stack",name:"modeText",ty:"wchar_t[128]"},database:"<active-session-id-from-current-idb_open>"}`, and `set_type {edits:{addr:"0x004b9670",kind:"stack",name:"color",ty:"unsigned int"},database:"<active-session-id-from-current-idb_open>"}`. No type write is authorized for `tileContext`; its existing `EPFTileContext` type must remain exact. Each response must contain one edit echoing the exact `addr`, `kind`, `name`, and `ty`, with `kind:"stack"`, `ok:true`, and no `error`, followed by a complete `stack_frame` reread. The A02 physical readback may use IDA's demonstrated canonical spelling `unsigned __int32`; no other type drift is accepted.
- Function types use only `set_type` with `kind:"function"` and `signature`: `set_type {edits:{addr:"0x00480e80",kind:"function",signature:"void __thiscall ChattingVarietyPane__OnPaint(ChattingVarietyPane *this)"},database:"<active-session-id-from-current-idb_open>"}` and `set_type {edits:{addr:"0x004b9670",kind:"function",signature:"void __thiscall GrafPort_SetTextBackFillColor(GrafPort *this, unsigned int color)"},database:"<active-session-id-from-current-idb_open>"}`. Each response must contain one exact echoed edit with `kind:"function"`, `ok:true`, and no `error`; immediate `lookup_funcs` plus complete `stack_frame` readback must show only the approved signature/frame deltas.
- Function regular comments use only `set_function_comments`: A01 is `set_function_comments {items:{addr:"0x00480e80",comment:"Source virtual ChattingVarietyPane::OnPaint. Draws CHATVAR.EPF frame 1 while g_pChattingVarietySelectPane exists, otherwise frame 0; labels g_pUserPane->m_currentSayMode as Talk/Shout/Whisper/Group/Clan and double-strikes centered text at x+1 then x."},database:"<active-session-id-from-current-idb_open>"}`; A02 is `set_function_comments {items:{addr:"0x004b9670",comment:"Source GrafPort::SetTextBackFillColor. Stores color in m_textBackFillColor at +0x78; this is not the +0x70 draw-mode setter."},database:"<active-session-id-from-current-idb_open>"}`. Each response must be exactly one no-error row for the requested `addr` with `function_addr` equal to the containing function start. Immediate `get_comments {addrs:"<function-address>",database:"<active-session-id-from-current-idb_open>"}` must show the exact function-regular text while address regular, address repeatable, and function repeatable remain at their literal prestates.
- A03 physical normalization uses exactly one no-name data-shaping request: `make_data {items:{addr:"0x0067addc",type:"ChattingVarietySelectPane *",delete_existing:true},database:"<active-session-id-from-current-idb_open>"}`. The `name` field is intentionally absent, and no `rename.batch.data`, global rename, or follow-up name write is authorized. Under the current endpoint this sequence applies the full declaration, derives size four, deletes the existing overlapping item range with expanded deletion, reapplies the same type, performs no name write, and clears cached Hex-Rays cfuncs once after successful shaping. Its sole accepted response is one no-error row with `addr:"0x0067addc"`, `name:""`, `type:"ChattingVarietySelectPane *"`, `size:4`, and `ok:true`. Immediate `inspect_items` over `0x0067add8`, `0x0067addc`, each interior byte `0x0067addd-0x0067addf`, and `0x0067ade0` must prove the new sole head `[0x0067addc,0x0067ade0)`, size four, data true, code false, empty physical name, exact pointer type, and unchanged adjacent heads. `get_bytes` over `[0x0067add8,0x0067ade4)` must preserve all bytes, including four zeros at the target. Complete `xrefs_to`/`xref_query` readback must preserve exactly the seven refs at `0x00480d7f`, `0x00480dc2`, `0x00480dc9`, `0x00480eb2`, `0x00481115`, `0x0048119d`, and `0x00483e04`; the names index must still contain exactly one `g_pChattingVarietySelectPane` at `0x0067addc`. No automatic-analysis wait, function/code creation, decompile, name-index delta, xref delta, or other analysis action is authorized. The single transient all-cfunc-cache clear encoded by `make_data` is the only accepted cache effect; any additional cache/analysis or structural side effect fails the action.
- A03 and A04 regular address comments use only `set_address_comments`, never broad `set_comments`: A03 is `set_address_comments {items:{addr:"0x0067addc",comment:"ChattingVarietySelectPane singleton. Non-null selects CHATVAR.EPF frame 1 in ChattingVarietyPane::OnPaint; null selects frame 0."},database:"<active-session-id-from-current-idb_open>"}`; A04 is `set_address_comments {items:{addr:"0x00614ee4",comment:"ChattingVarietyPane primary-vtable OnPaint slot -> ChattingVarietyPane__OnPaint (0x00480e80)."},database:"<active-session-id-from-current-idb_open>"}`. Each response must be exactly one no-error row with the requested `addr`. Immediate `get_comments` at the exact address must show only the exact regular-address text; repeatable address comments remain absent, and A03 item/name/type/bytes/xrefs plus A04 item/name/type/bytes/value/xref/adjacent slots remain exact.
- Actions are serial in this order: A01 function dry/actual rename; A01 stack dry/actual rename and any listed stack type, one row at a time; A01 function type; A01 function regular comment; complete A01 readback; then the same A02 order; then A03 `make_data`, complete A03 item/bytes/name/xref readback, A03 `set_address_comments`, complete A03 readback; then A04 `set_address_comments` and complete A04/adjacent-slot readback. Stop before the next action on any schema, response, pre-state, collision, or protected-state mismatch.
...
- Preserve target range `[0x00480e80,0x00480ff1)`, successor at `0x00481000`, and the intervening fifteen `0xcc` bytes outside the function.
- Freeze A01's reachability at exactly one inbound data xref `0x00614ee4 -> 0x00480e80` and zero ordinary code callers; preserve its body/range/bytes and all 27 existing internal regular comments. Permit only the literal listed deltas in A01-NAME, A01-LOCAL-THIS, A01-LOCAL-CONTEXT, A01-LOCAL-TEXT, A01-TYPE, and A01-COMMENT; freeze every unlisted frame row, name, type/prototype field, comment channel, byte, range, xref, and caller state.
- Freeze A02's complete 11-reference set at `0x00467be6`, `0x00467e26`, `0x00467ec4`, `0x00480ead`, `0x0048216f`, `0x004b1c7b`, `0x00550e87`, `0x00550fe7`, `0x0057c1a5`, `0x00585bf5`, and `0x005bc820`, plus the exact 13-byte body, saved-register row, return-address row, and every other unlisted prestate. Permit only the literal listed deltas in A02-NAME, A02-LOCAL, A02-TYPE, and A02-COMMENT; freeze every other frame row, name, type/prototype field, comment channel, byte, range, and xref state.
- Preserve A03's four zero bytes, seven xrefs, one semantic singleton name, and physically empty item-name channel.
- Preserve A04's four bytes, value, one data xref, empty name/type, and absent repeatable comment.
- Protect adjacent vtable cells `0x00614edc` and `0x00614ee8`, neighboring singleton storage `0x0067add8` and `0x0067ade0`, `g_pEPFLib`, `g_pUserPane`, `EPFTileContext_Initialize`, `ResourceLayoutTable_LookupLayoutEntry`, `GrafPort_RenderTileFrame`, `_wcscpy_s`, `GrafPort_SetTextColor`, `GrafPort_MoveTo`, and `GrafPort_DrawWideText`; no action is recommended for these dependencies.
- Any current pre-state drift, collision, unexpected frame/type/comment delta, changed bytes/range/xrefs, or inability to identify the exact current session/path makes the affected row unsafe/stale and requires supervisor stop-and-reread rather than adaptation by guess.

### Supervisor Operational Boundary

The supervisor must perform MCP availability checks, runtime attestation, session creation, backup/save policy, process handling, direct action application, fresh-session readback, persistence classification, and any rollback outside this report. No action receives credit from a response label alone: each accepted row requires the expected physical readback above, protected no-change comparison, and supervisor-owned lifecycle evidence. C2FD-031 through C2FD-038 remain pending.

### Historical Format Disposition

Earlier revisions embedded V1-V6 PowerShell/C#/fixture transaction packages and recorded their parser, hostile, process, and recovery receipts. The superseding report-format rule removes those executable packages and withdraws every claim supported only by running them. Their existence, repeated Gate 1 failures, stopped encoded/unbounded runs, and dated pre-save Gate 2B rollback attempts remain historical process facts, but none is current acceptance evidence. Ordinary binary research, documentation callback evidence, formal C++, exact proposed IDA pre/post states, and the three dated no-persistence Gate 2B failures remain independently supported and retained.
`````

## Additional removed operational lines: C2FD mirrored ledger/checklist payload cleanup

> NON-AUTHORITATIVE ARCHIVE. The exact lines below were removed from the source and must never be executed.

`````text
| C2FD-031 | 0002FD | Apply/read back only A01-NAME, A01-LOCAL-THIS, A01-LOCAL-CONTEXT, A01-LOCAL-TEXT, A01-TYPE, and A01-COMMENT through literal `rename.batch.func` dry/actual requests with `pure:true`, three isolated `rename.batch.stack` dry/actual pairs with `pure:false`, exact `set_type` `kind:"stack"`/`kind:"function"` fields, and exact `set_function_comments`; use only the database returned by no-analysis `idb_open`, and preserve exactly one inbound data xref `0x00614ee4 -> 0x00480e80` with zero ordinary code callers before and after every step. | High | body/frame/pre-state/class ownership and current endpoint contracts | Canonical IDB / A01 | Primary supervisor Gate 2B | incorporate | proposed |
| C2FD-032 | 0002FD | After every A01 stack rename/type request, reread the complete physical frame through `stack_frame`: require `thisPane` `ChattingVarietyPane *` at `+0xc`, `tileContext` `EPFTileContext` at `+0x10`, `modeText` `wchar_t[128]` at `+0x38..+0x138`, and preserve every unlisted row/byte. | High | occupied frame/body/readback and narrow stack schemas | Canonical IDB / A01 frame | Primary supervisor Gate 2B | incorporate | proposed |
| C2FD-033 | 000162 | Apply/read back only A02-NAME, A02-LOCAL, A02-TYPE, and A02-COMMENT through literal `rename.batch.func` dry/actual `pure:true`, isolated `rename.batch.stack` dry/actual `pure:false`, `set_type` stack/function requests, and `set_function_comments`; preserve the exact 11 inbound references at `0x00467be6`, `0x00467e26`, `0x00467ec4`, `0x00480ead`, `0x0048216f`, `0x004b1c7b`, `0x00550e87`, `0x00550fe7`, `0x0057c1a5`, `0x00585bf5`, and `0x005bc820`, the exact 13-byte body, three-row frame, physical `unsigned __int32` normalization, and every other unlisted prestate. | High | exact 11-reference set/helper/callers/GrafPort/frame/prototype and current endpoint contracts | Canonical IDB / A02 | Primary supervisor Gate 2B | incorporate | proposed |
| C2FD-034 | 0002XA | Apply exact no-name `make_data {items:{addr:"0x0067addc",type:"ChattingVarietySelectPane *",delete_existing:true},database:"<active-session-id-from-current-idb_open>"}`, then `set_address_comments {items:{addr:"0x0067addc",comment:"ChattingVarietySelectPane singleton. Non-null selects CHATVAR.EPF frame 1 in ChattingVarietyPane::OnPaint; null selects frame 0."},database:"<active-session-id-from-current-idb_open>"}`; require sole four-byte head/data/type, empty physical name, one semantic name, unchanged zeros/seven refs/adjacent heads, no analysis action, and only the endpoint's one transient all-cfunc-cache clear. | High | item/global/bytes/xrefs, demonstrated no-name endpoint behavior, and current schemas | Canonical IDB / A03 | Primary supervisor Gate 2B | incorporate | proposed |
| C2FD-035 | 00001Y | Apply only `set_address_comments {items:{addr:"0x00614ee4",comment:"ChattingVarietyPane primary-vtable OnPaint slot -> ChattingVarietyPane__OnPaint (0x00480e80)."},database:"<active-session-id-from-current-idb_open>"}`; immediate `get_comments` and item/byte/xref reads must preserve repeatable absence, slot value, target ref, and adjacent slots. | High | exact slot/body and narrow address-comment endpoint | Canonical IDB / A04 | Primary supervisor Gate 2B | incorporate | proposed |
| C2FD-036 | 0002FD | Protect every unlisted function/frame/prototype/comment/item/name/string/byte/xref/UDT/adjacent global or slot; permit only A01-NAME/A01-LOCAL-THIS/A01-LOCAL-CONTEXT/A01-LOCAL-TEXT/A01-TYPE/A01-COMMENT while freezing A01's sole data xref `0x00614ee4 -> 0x00480e80` and zero ordinary code callers, only A02-NAME/A02-LOCAL/A02-TYPE/A02-COMMENT while freezing A02's literal 11-reference set and other prestates, and only the literal listed A03/A04 deltas; allow no broad `set_comments`, `rename.batch.local`, data rename, decompile, auto-analysis, `/vibe/`, or unlisted cache effect beyond A03's one expected cfunc-cache clear. | Exact | complete pre-state, endpoint implementation, and no-change contract | Canonical IDB / protections | Primary supervisor Gate 2B | already-present | proposed |
| C2FD-037 | 0002FD | Bind authority only from literal `idb_open` with `run_auto_analysis:false`; before each serial action require current schema, exact pre-state, and zero collisions, then require the literal endpoint response and complete narrow readback; stop on mismatch, partial/error result, extra effect, or protected-state drift. | Exact | Section-21 endpoint binding/table/no-change state | Canonical IDB / collision/readback | Primary supervisor Gate 2B | incorporate | proposed |
| C2FD-038 | 0002FD | Grant IDA credit only after the primary supervisor directly applies the exact Section-21 endpoint sequence, independently verifies all immediate and fresh persisted readbacks under supervisor-owned backup/save/process/persistence/rollback safeguards, or proves exact unchanged pre-state after no-save/rollback; report text/removed packages supply no credit. | Exact | literal Section-21 handoff, dated live evidence, no-save histories, direct verification requirement | Canonical IDB / Gate 2B | Primary supervisor Gate 2B | incorporate | proposed |
| C2FD-031 | 0002FD | Primary supervisor Gate 2B | incorporate | [ ] Apply/read back only A01-NAME, A01-LOCAL-THIS, A01-LOCAL-CONTEXT, A01-LOCAL-TEXT, A01-TYPE, and A01-COMMENT through literal `rename.batch.func` dry/actual requests with `pure:true`, three isolated `rename.batch.stack` dry/actual pairs with `pure:false`, exact `set_type` `kind:"stack"`/`kind:"function"` fields, and exact `set_function_comments`; use only the database returned by no-analysis `idb_open`, and preserve exactly one inbound data xref `0x00614ee4 -> 0x00480e80` with zero ordinary code callers before and after every step. | proposed |
| C2FD-032 | 0002FD | Primary supervisor Gate 2B | incorporate | [ ] After every A01 stack rename/type request, reread the complete physical frame through `stack_frame`: require `thisPane` `ChattingVarietyPane *` at `+0xc`, `tileContext` `EPFTileContext` at `+0x10`, `modeText` `wchar_t[128]` at `+0x38..+0x138`, and preserve every unlisted row/byte. | proposed |
| C2FD-033 | 000162 | Primary supervisor Gate 2B | incorporate | [ ] Apply/read back only A02-NAME, A02-LOCAL, A02-TYPE, and A02-COMMENT through literal `rename.batch.func` dry/actual `pure:true`, isolated `rename.batch.stack` dry/actual `pure:false`, `set_type` stack/function requests, and `set_function_comments`; preserve the exact 11 inbound references at `0x00467be6`, `0x00467e26`, `0x00467ec4`, `0x00480ead`, `0x0048216f`, `0x004b1c7b`, `0x00550e87`, `0x00550fe7`, `0x0057c1a5`, `0x00585bf5`, and `0x005bc820`, the exact 13-byte body, three-row frame, physical `unsigned __int32` normalization, and every other unlisted prestate. | proposed |
| C2FD-034 | 0002XA | Primary supervisor Gate 2B | incorporate | [ ] Apply exact no-name `make_data {items:{addr:"0x0067addc",type:"ChattingVarietySelectPane *",delete_existing:true},database:"<active-session-id-from-current-idb_open>"}`, then `set_address_comments {items:{addr:"0x0067addc",comment:"ChattingVarietySelectPane singleton. Non-null selects CHATVAR.EPF frame 1 in ChattingVarietyPane::OnPaint; null selects frame 0."},database:"<active-session-id-from-current-idb_open>"}`; require sole four-byte head/data/type, empty physical name, one semantic name, unchanged zeros/seven refs/adjacent heads, no analysis action, and only the endpoint's one transient all-cfunc-cache clear. | proposed |
| C2FD-035 | 00001Y | Primary supervisor Gate 2B | incorporate | [ ] Apply only `set_address_comments {items:{addr:"0x00614ee4",comment:"ChattingVarietyPane primary-vtable OnPaint slot -> ChattingVarietyPane__OnPaint (0x00480e80)."},database:"<active-session-id-from-current-idb_open>"}`; immediate `get_comments` and item/byte/xref reads must preserve repeatable absence, slot value, target ref, and adjacent slots. | proposed |
| C2FD-036 | 0002FD | Primary supervisor Gate 2B | already-present | [ ] Protect every unlisted function/frame/prototype/comment/item/name/string/byte/xref/UDT/adjacent global or slot; permit only A01-NAME/A01-LOCAL-THIS/A01-LOCAL-CONTEXT/A01-LOCAL-TEXT/A01-TYPE/A01-COMMENT while freezing A01's sole data xref `0x00614ee4 -> 0x00480e80` and zero ordinary code callers, only A02-NAME/A02-LOCAL/A02-TYPE/A02-COMMENT while freezing A02's literal 11-reference set and other prestates, and only the literal listed A03/A04 deltas; allow no broad `set_comments`, `rename.batch.local`, data rename, decompile, auto-analysis, `/vibe/`, or unlisted cache effect beyond A03's one expected cfunc-cache clear. | proposed |
| C2FD-037 | 0002FD | Primary supervisor Gate 2B | incorporate | [ ] Bind authority only from literal `idb_open` with `run_auto_analysis:false`; before each serial action require current schema, exact pre-state, and zero collisions, then require the literal endpoint response and complete narrow readback; stop on mismatch, partial/error result, extra effect, or protected-state drift. | proposed |
| C2FD-038 | 0002FD | Primary supervisor Gate 2B | incorporate | [ ] Grant IDA credit only after the primary supervisor directly applies the exact Section-21 endpoint sequence, independently verifies all immediate and fresh persisted readbacks under supervisor-owned backup/save/process/persistence/rollback safeguards, or proves exact unchanged pre-state after no-save/rollback; report text/removed packages supply no credit. | proposed |
`````

## Additional removed operational lines: B006 UID0002FD residual orchestration cleanup

> NON-AUTHORITATIVE ARCHIVE. The exact lines below were removed from the source and must never be executed.

`````text
Section 21 is a handoff, not an executable plan. Supervisor must independently confirm database/session identity, literal pre-state, collisions, worker safety, and affected extent. For A01, only A01-NAME, A01-LOCAL-THIS, A01-LOCAL-CONTEXT, A01-LOCAL-TEXT, A01-TYPE, and A01-COMMENT are permitted while the sole data xref `0x00614ee4 -> 0x00480e80` and zero ordinary code callers remain frozen. For A02, only A02-NAME, A02-LOCAL, A02-TYPE, and A02-COMMENT are permitted while the literal 11-reference set and all other unlisted prestates remain frozen. Apply only the accepted A03/A04 rows directly, and verify every expected readback and protected no-change state. Backup, save, processes, persistence, rollback, and cleanup remain outside this report.
3. If accepted, supervisor applies only A01-NAME/A01-LOCAL-THIS/A01-LOCAL-CONTEXT/A01-LOCAL-TEXT/A01-TYPE/A01-COMMENT while freezing A01's sole data xref and zero ordinary code callers, only A02-NAME/A02-LOCAL/A02-TYPE/A02-COMMENT while freezing A02's literal 11-reference set and all other prestates, and only the listed A03/A04 rows; all work occurs directly outside the report with supervisor-owned backup/save/readback/process/persistence/rollback safeguards, and credit requires literal readback plus every protected invariant.
`````
