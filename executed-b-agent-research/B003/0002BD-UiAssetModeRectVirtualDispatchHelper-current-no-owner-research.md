...
- Confidence: high for the no-owner/non-emitting recommendation. The helper is source-shaped project code, but current IDA MCP and raw PE evidence still expose no caller, table entry, vtable slot, direct branch/call target, pointer, class-private field tie, or source-use output route.

This is a legitimate reconstructable no-owner/non-emitting item. The best owner lead remains `MapNamePane`, but the evidence is associative only. It is not strong enough to assign a canonical owner or emitter under the current owner/emitter model.
...
- Source queue/report row: `tools/leaser/Agents/Supervisor_notes.md` current `B003-noowner-current-0002BD` row, plus `tools/leaser/Agents/no_owner_b-agent-tracker.md` active 2026-06-14 no-owner memory sweep row.
...
- `tools/leaser/Agents/Agent-B001/research/executed/0002BD-UiAssetModeRectVirtualDispatchHelper-restart-no-owner-research.md`
- `tools/leaser/Agents/Agent-B003/research/executed/0002BD-UiAssetModeRectVirtualDispatchHelper-current-no-owner-sweep.md`
- Earlier executed B003/B002 reports found by filename search.

## Executive Recommendation

Leave the target no-owner/non-emitting.

The code body at `0x00503060-0x005030bc` is a real source-shaped helper: it receives `this` in `ecx`, optionally creates a local `RectBounds(0, 0, 0xa6/0xad, 0x10)` depending on [UID:0000SW] `g_useEpfAssets`, and calls through `this->vtable + 0x20`. The current blocker is not behavior comprehension; it is source-route evidence. No current evidence identifies the receiver class or an output file route.

`MapNamePane` is the best hypothesis if forced, because the helper is adjacent to the MapName/MiniMapButton island and uses the `0xad` width also documented as the 173-pixel MapName clamp. That is still insufficient. Unlike sibling [UID:0002BF], this helper does not read the MapName-owned `this + 0x220` font-table field, has no MapName vtable slot, and has no caller from the MapName constructor/render/update/destructor family.
...
The supervisor assignment asked Agent-B003 to recheck this current generated no-owner/non-emitting target from scratch using current documentation and IDA MCP evidence, and to decide whether it should remain `CANONICAL_OWNER:NONE` with blank `EMITTER_UIDS`, gain owner/emitter routing, be split/merged/reclassified, or require repair.
...
The current `Supervisor.md` constraints require legitimate documentation improvement and evidence-backed assignment only; agents must not fill owner/emitter fields merely to reduce no-owner/non-emitting totals. `by-structure.md` adds the controlling owner/emitter rules:
...
`inference_research.md` shaped the recommendation by treating address adjacency, shared globals, and broad callee dependencies as leads rather than ownership proof. The current docs and prior reports were useful search guides, but current IDA and PE evidence drove the final decision.
...
- Candidate owner docs: [UID:00007P] `MapNamePane`, [UID:0000L2] `MapNamePane` file, [UID:00008B] `MiniMapButtonPane`, [UID:0000LE] `MiniMap`, [UID:0000SW] `g_useEpfAssets`, and [UID:00015S] `RectGeometryHelpers`.
- Generated and shared coverage rows.
- Live IDA MCP session `b001_0002bd`: `server_health`, `lookup_funcs`, `xrefs_to`, `get_bytes`, `find_bytes`, `make_signature`, `make_signature_for_range`, `search_text`, and `find_regex`.
...
Target raw bytes from `get_bytes 0x00503050 size 0x100` confirm:
...
| `0x005030a0` | internal code xrefs from `0x0050309d` and `0x00503077` | internal branch target |
...
- `make_signature_for_range 0x00503060-0x005030bc`: unique range signature for the whole helper body.

Other IDA facts:

- `search_text` scoped to `0x00503000-0x00503200` finds `byte_66DA97` compares only at `0x00503079` and sibling `0x005030c9`.
- `find_regex '(?i)\.pdb|RSDS|NB10|\.cpp|\.cxx'` returns no source/debug metadata matches.
...
The branch scan was rerun after correcting PowerShell signed-byte handling; the completed result finds the expected internal branch and nearby thunk controls, but no external branch/call to the target entry.

## Function / Child Inventory

| Range / Item | UID / Path | Role | Reconstructable | Direct Parent | Score | Status |
| --- | --- | --- | --- | --- | --- | --- |
| `0x00502e30-0x0050305c` | `0x00502e10-0x0050305c.MainMenuAnsiTextDialogPacketHelpers.md` area | left neighboring modeled helper | true | outside scope | current docs | ends before padding |
...
| `0x00503110-0x005031e1` | [UID:0002BF] `WideGlyphResourceTextRendererRaw` | sibling glyph renderer | true | [UID:00007P] MapNamePane | `87/91` | assigned by `this+0x220` field evidence |
...
- Sibling [UID:0002BF] is MapName-owned because it uniquely reads the MapName-owned `this + 0x220` font resource field; this contrast is important negative evidence for `0002BD`.
...
- [UID:00008B] and [UID:0000LE] prove `MiniMapButtonPane`/`MiniMap` is real and gate-valid, but current IDA/PE does not connect this helper to it.
...
Current generated row in `auto-generated/-ag-memory-coverage.md`:
...
The shared coverage row cites an older MCP session ID but remains substantively correct. No supervisor-owned row change is required.
...
- [UID:0002BF], the third raw helper in the same physical cluster, now has MapName ownership.
...
- The target does not read MapName's `this + 0x220` field, unlike sibling [UID:0002BF].
...
### 8. Split, merge, reclassify, or rename
...
- Reject split/merge/reclassification/rename.
...
- No replacement required. Retain the current row quoted above in `Documentation Evidence And IDA Status`.
...
<!-- {"agent":"B003","command_id":"000000004165","destination_path":"executed-b-agent-research/B003/0002BD-UiAssetModeRectVirtualDispatchHelper-current-no-owner-research.md","details":"legacy registry reconstruction","event":"executed","timestamp":"2026-06-23T21:10:27","uid":"0002BD"} -->
