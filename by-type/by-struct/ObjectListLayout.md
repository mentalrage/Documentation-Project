...
*** AUTOGEN_PARENT_UID:00009Q | ONLY MODIFY VALUE - DO NOT REMOVE!!! ***
...
- Autogen parent: attached to [UID:00009Q][ObjectList](by-class/ObjectList.md); the class scores `84/88` and this layout scores `82/88`, so both sides satisfy the 80/80 parent gate.
- Evidence basis: IDA MCP decompilation of `ObjectList` lifecycle/accessor methods and the linked ObjectList accessor/sweep plus MapPaneSpatialIndex compatibility-alias documentation.
...
- [UID:0001D2][0x00532530-0x0053272e.MapPaneSpatialIndex](by-memory/0x00532530-0x0053272e.MapPaneSpatialIndex.md) is now documented as a compatibility alias subset over these ObjectList fields, not as a separate allocated layout.

## Naming Notes

The `primary/secondary/tertiary` names are structural placeholders. The compatibility `MapPaneSpatialIndex` docs still preserve render-layer and row-bucket search vocabulary for the same fields. Keep both naming surfaces linked until render-layer callers define final source names.
