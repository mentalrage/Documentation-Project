...
- Reconstructable: true. This is a NexusTK-authored tree node record required by `FolderTreePane`; autogen parent/code remains blank until the final tree-template source form is stronger.
...
| `0x08` | 4 | `lastChildOrPrevIndex` | Set with sibling/child linkage during insert; exact role still needs naming. |
| `0x0c` | 4 | `prevSiblingOrReserved` | Link/state field copied by storage helpers. |
...
| `0x1c` | 1 | `hasLoadedChildren` | Mouse expand/collapse logic checks a loaded-child flag near this offset. |
| `0x1d` | 1 | `isExpanded` | `CollapseNode` clears `node + 0x1d`; key/mouse logic reads the same flag. IDA recheck confirms this exact byte. |
