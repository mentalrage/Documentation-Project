...
| primary `0x0062e584` | `+0x5c` | `0x00599ba0` | `UrlAlertPane::OnConfirm`, opens the stored URL and optionally exits. |
| primary `0x0062e584` | `+0x60` | `0x0041b6a0` | Base no-op/guard secondary callback slot. |
| secondary `0x0062e5ec` | `+0x00` | `0x00599bdb` | Adjustor thunk into scalar deleting destructor with `this - 0xa0`. |
| tertiary `0x0062e61c` | `+0x00` | `0x00599be6` | Adjustor thunk into scalar deleting destructor with `this - 0xa4`. |
...
Model `UrlAlertPane` as an `AlertPane` subclass with three vtable views at `+0x00`, `+0xa0`, and `+0xa4`. The class-specific data extends the base alert object with a heap-owned URL pointer at `+0x270` and an exit-after-open byte at `+0x274`.
